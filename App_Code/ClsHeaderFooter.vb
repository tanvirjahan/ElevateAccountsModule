...
Public Class ClsHeaderFooter : Implements IPdfPageEvent
    ...
    Dim documentwidth As Integer
    Dim titletable As PdfPTable = Nothing
    Dim tableheader As PdfPTable = Nothing
    Dim FooterTableline As PdfPTable = Nothing
    Dim Isfilename As String = Nothing
    Dim FooterTable As PdfPTable = Nothing
    Public Sub New(ByVal companyname As PdfPTable, ByVal common As PdfPTable, ByVal footer As PdfPTable, ByVal header As PdfPTable, Optional ByVal fileName As String = "", Optional ByVal FooterUndreline As PdfPTable = Nothing)
        ...
        If footer IsNot Nothing Then
            FooterTable = footer
        End If
        FooterTableline = FooterUndreline
        Isfilename = fileName
        ' documentwidth = PageSize
    End Sub
    ...
    Public Sub OnStartPage(ByVal writer As PdfWriter, ByVal document As Document) Implements IPdfPageEvent.OnStartPage
        Try
            ...
            'If documentwidth <> 1 Then
            '    document.SetPageSize(iTextSharp.text.PageSize.A4.Rotate)
            'End If

            If writer.PageNumber >= 0 Then

                If titletable IsNot Nothing Then
                    document.Add(titletable)
                End If
                If Reporttitle IsNot Nothing Then
                ...
                ElseIf Isfilename = "printDate" Then
                    FooterTable.WriteSelectedRows(0, -1, 25, 19, writer.DirectContent)
                ElseIf Isfilename = "Ageing Summary" Then
                    FooterTable.WriteSelectedRows(0, -1, 30, 40, writer.DirectContent)
                ElseIf Isfilename = "freeform" Then
                    ' Footer with Horizonatal line 
                    ' If FooterTableline IsNot Nothing Then
                    FooterTableline.WriteSelectedRows(0, -1, 30, 50, writer.DirectContent)
                    'Else
                    ' Footer withiout Horizonatal line 
                    ' FooterTable.WriteSelectedRows(0, -1, 30, 20, writer.DirectContent)
                    ' End If
                End If
    ...
    Public Sub OnEndPage(ByVal writer As PdfWriter, ByVal document As Document) Implements IPdfPageEvent.OnEndPage
        Try
            ...
            If Isfilename = "ConsolidateInvoice" Then
                ...
                cb.ShowTextAligned(PdfContentByte.ALIGN_CENTER, "Print Date : " + Date.Now.ToString("yyyy-MM-dd HH:mm:ss"), 90, 20, 0)
                cb.ShowTextAligned(PdfContentByte.ALIGN_CENTER, "Page No. : " + Str(document.PageNumber), 545, 20, 0)
                cb.EndText()
            End If
