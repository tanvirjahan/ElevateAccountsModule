...
Partial Class AcctCodesNewMapping
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Page.IsPostBack = False Then
            ...
            Try
                ...
                RowsPerPageCUS.SelectedValue = 10
                ...
                Else
                    ...
                    objUser.CheckUserRight(Session("dbconnectionName"), CType(Session("GlobalUserName"), String), CType(Session("Userpwd"), String),
                                           CType(strappname, String), "AccountsModule\AcctCodesNewMapping.aspx?appid=" + AppId, btnAddNew, btnExportToExcel,
                ...
                '--------end
                Session.Add("strsortExpression", "an.acctcode")
            ...
            Catch ex As Exception
                ...
                objUtils.WritErrorLog("AcctCodesNewMapping.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
            End Try
        End If
...
#Region "Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = 'ASC')"
    Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = "ASC")
        ...
        strSqlQry = ""
        Try
            strSqlQry = "SELECT bankcode,bankname, accountname , " &
        ...
        For i = 0 To lsMainArr.GetUpperBound(0)
            Select Case lsMainArr(i).Split(":")(0).ToString.ToUpper.Trim
                ...
                Case "ACCOUNTCODE"
                    ...
                    sbAddToDataTable("AccountCode", lsProcessCity, "AC")
                Case "ACCOUNTNAME"
                    lsProcessCity = lsMainArr(i).Split(":")(1)
                    sbAddToDataTable("AccountName", lsProcessCity, "AN")
                Case "MAPPEDACCOUNTCODE"
                    lsProcessCity = lsMainArr(i).Split(":")(1)
                    sbAddToDataTable("MappedAccountCode", lsProcessCity, "MAC")
                Case "MAPPEDACCOUNTNAME"
                    lsProcessCity = lsMainArr(i).Split(":")(1)
                    sbAddToDataTable("MappedAccountName", lsProcessCity, "MAN")
                Case "TEXT"
...
#Region " Protected Sub gv_SearchResult_Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs) Handles gv_SearchResult.Sorting"
    ...
    '                    If "AccountCode" = dtDynamics.Rows(j)("code").ToString.Trim.ToUpper Then
    ...
    '                        lblAccountname.Text = Regex.Replace(lblAccountname.Text.Trim, lsCurrencyName.Trim(),
    '                            Function(match As Match) String.Format("<span style = 'background-color:#ffcc99'>{0}</span>", match.Value),
    '                                        RegexOptions.IgnoreCase)
    '                    End If
    '                    If lsGroupName.Trim <> "" Then
    '                        lblbankname.Text = Regex.Replace(lblbankname.Text.Trim, lsGroupName.Trim(),
    '                            Function(match As Match) String.Format("<span style = 'background-color:#ffcc99'>{0}</span>", match.Value),
    '                                        RegexOptions.IgnoreCase)
    '                    End If

    '                    If lsTextName.Trim <> "" Then
    '                        lblAccountname.Text = Regex.Replace(lblAccountname.Text.Trim, lsTextName.Trim(),
    '                          Function(match As Match) String.Format("<span style = 'background-color:#ffcc99'>{0}</span>", match.Value),
    '                                      RegexOptions.IgnoreCase)
    '                        lblbankname.Text = Regex.Replace(lblbankname.Text.Trim, lsTextName.Trim(),
    '                         Function(match As Match) String.Format("<span style = 'background-color:#ffcc99'>{0}</span>", match.Value),
    '                                     RegexOptions.IgnoreCase)
    '                    End If
...
#End Region
    ...
    Private Sub FillGridNew()
        ...
        Dim strMappedAccountCode As String = ""
        Dim strMappedAccountName As String = ""
        Dim strTextValue As String = ""
        Dim strBindCondition As String = ""
        Try
            If dtt.Rows.Count > 0 Then
                For i As Integer = 0 To dtt.Rows.Count - 1
                    If dtt.Rows(i)("Code").ToString = "ACCOUNTCODE" Then
                        If strAccountCode <> "" Then
                            strAccountCode = strAccountCode + ",'" + dtt.Rows(i)("Value").ToString + "'"
                        Else
                            strAccountCode = "'" + dtt.Rows(i)("Value").ToString + "'"
                        End If
                    End If
                    If dtt.Rows(i)("Code").ToString = "ACCOUNTNAME" Then
                        If strAccountName <> "" Then
                            strAccountName = strAccountName + ",'" + dtt.Rows(i)("Value").ToString + "'"
                        Else
                            strAccountName = "'" + dtt.Rows(i)("Value").ToString + "'"
                        End If
                    End If
                    If dtt.Rows(i)("Code").ToString = "MAPPEDACCOUNTCODE" Then
                        If strMappedAccountCode <> "" Then
                            strMappedAccountCode = strMappedAccountCode + ",'" + dtt.Rows(i)("Value").ToString + "'"
                        Else
                            strMappedAccountCode = "'" + dtt.Rows(i)("Value").ToString + "'"
                        End If
                    End If
                    If dtt.Rows(i)("Code").ToString = "MAPPEDACCOUNTNAME" Then
                        If strMappedAccountName <> "" Then
                            strMappedAccountName = strMappedAccountName + ",'" + dtt.Rows(i)("Value").ToString + "'"
                        Else
                            strMappedAccountName = "'" + dtt.Rows(i)("Value").ToString + "'"
                        End If
                    End If
            ...
            strBindCondition = BuildConditionNew(strAccountCode, strAccountName, strMappedAccountCode, strMappedAccountName, strTextValue)
            Dim myDS As New DataSet
            ...
            'strSqlQry = "SELECT bankcode,bankname, accountname ,accountcurrency,accountnumber,case when bankdetails_master.active=1 then ' Active' else 'In Active' end  active," &
            ...
            strSqlQry = "select an.acctcode AccountCode,an.acctname AccountName,am.acctcode_newbs MappedAccountCode,ag.acctname MappedAccountName " &
                "from acctmast an " &
                "left outer join acctmast_newbsmapping am on am.acctcode = an.acctcode and am.div_code = an.div_code " &
                "left outer join acctgroup_newBS ag on am.acctcode_newbs = ag.acctcode and am.div_code = ag.div_code "

            If strBindCondition <> "" Then
                strSqlQry = strSqlQry & " WHERE an.div_code='" & ViewState("divcode") & "' and  " & strBindCondition & " ORDER BY " & strorderby & " " & strsortorder
            Else
                strSqlQry = strSqlQry & " WHERE an.div_code='" & ViewState("divcode") & "' ORDER BY " & strorderby & " " & strsortorder
            End If
            SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))                             'Open connection
            myDataAdapter = New SqlDataAdapter(strSqlQry, SqlConn)
            myDataAdapter.Fill(myDS)
    ...
    Private Function BuildConditionNew(ByVal strAccountCode As String, ByVal strAccountName As String, ByVal strMappedAccountCode As String, ByVal strMappedAccountName As String, ByVal strTextValue As String) As String
        strWhereCond = ""

        If strAccountCode.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                strWhereCond = "upper(an.acctcode) IN (" & Trim(strAccountCode.Trim.ToUpper) & ")"
            Else
                strWhereCond = strWhereCond & " AND upper(an.acctcode) IN (" & Trim(strAccountCode.Trim.ToUpper) & ")"
            End If
        End If

        If strAccountName.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                strWhereCond = "upper(an.acctname) IN (" & Trim(strAccountName.Trim.ToUpper) & ")"
            Else
                strWhereCond = strWhereCond & " AND upper(an.acctname) IN (" & Trim(strAccountName.Trim.ToUpper) & ")"
            End If
        End If

        If strMappedAccountCode.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                strWhereCond = "upper(am.acctcode_newbs) IN (" & Trim(strMappedAccountCode.Trim.ToUpper) & ")"
            Else
                strWhereCond = strWhereCond & " AND upper(am.acctcode_newbs) IN (" & Trim(strMappedAccountCode.Trim.ToUpper) & ")"
            End If
        End If

        If strMappedAccountName.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                strWhereCond = "upper(ag.acctname) IN (" & Trim(strMappedAccountName.Trim.ToUpper) & ")"
            Else
                strWhereCond = strWhereCond & " AND upper(ag.acctname) IN (" & Trim(strMappedAccountName.Trim.ToUpper) & ")"
            End If
        End If
            ...
            For i = 0 To lsMainArr.GetUpperBound(0)
                ...
                If strValue <> "" Then
                    If Trim(strWhereCond1) = "" Then
                        strWhereCond1 = "upper(an.acctcode) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%'  or  upper(an.acctname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or   upper(am.acctcode_newbs) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or   upper(ag.acctname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%'  "
                    Else
                        strWhereCond1 = strWhereCond1 & " OR  upper(an.acctcode) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or  upper(an.acctname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or   upper(am.acctcode_newbs) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or   upper(ag.acctname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%'  "
                    End If
                End If
            Next
    ...
    Protected Sub btnExportToExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExportToExcel.Click
        ...
        Try
            If gv_SearchResult.Rows.Count <> 0 Then
                ...
                strSqlQry = "SELECT bankcode AS [Bank TypeCode],bankname AS [Bank  Name]," &
                            " accountname as [Account Name] , accountcurrency as [Account Currency],accountnumber as [Account Number],ibannumber as [IBAN Number],swiftcode as [Swift Code]," &
                            " (Convert(Varchar, Datepart(DD,adddate))+ '/'+ Convert(Varchar, Datepart(MM,adddate))+ '/'+ Convert(Varchar, Datepart(YY,adddate)) + ' ' + Convert(Varchar, Datepart(hh,adddate))+ ':' + Convert(Varchar, Datepart(m,adddate))+ ':'+ Convert(Varchar, Datepart(ss,adddate))) as [Date Created]," &
                            " adduser as [User Created],(Convert(Varchar, Datepart(DD,moddate))+ '/'+ Convert(Varchar, Datepart(MM,moddate))+ '/'+ Convert(Varchar, Datepart(YY,moddate)) + ' ' + Convert(Varchar, Datepart(hh,moddate))+ ':' + Convert(Varchar, Datepart(m,moddate))+ ':'+ Convert(Varchar, Datepart(ss,moddate))) as [Date Modified],moduser as [User Modified] " &
...
#Region "Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click"
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        Try
            ...
            Session.Add("BackPageName", "AcctCodesNewMapping.aspx")
            ...
            strpop = "window.open('rptReportNew.aspx?Pageame=BankDetails&BackPageName=AcctCodesNewMapping.aspx&divid=" & ViewState("divcode") & "','BankTypeMast');"
...
#End Region
    ...
    Public Shared Function GetAccountslist(ByVal prefixText As String, ByVal count As Integer, ByVal contextKey As String) As List(Of String)
        Dim strSqlQry As String = ""
        Dim myDS As New DataSet
        ...
        Try
            ...
            'End If
            'changed by mohamed / ram on 22/08/2022
            strSqlQry = "select acctcode,acctname from acctgroup_newBS where ((acctlevel = 3 and acctorder not in (3,4,6)) or (acctlevel = 1 and acctorder in (3,4,6))) and div_code = '01' and acctname like  '%" & Trim(prefixText) & "%'"
        ...
        Try
            ...
            For Each gvrow In gv_SearchResult.Rows
                If gvrow.RowType = DataControlRowType.DataRow Then
                    Dim bf As CheckBox = (TryCast(gvrow.Cells(0).FindControl("chkrow"), CheckBox))
                    Dim AccCode As Label = (TryCast(gvrow.cells(1).FindControl("lblAccCode"), Label))
                    If bf.Checked Then
                        Dim strAccCode = AccCode.Text.Trim()
                        strxml += "<ACCDET>"
                        strxml += "<ACCCODE>" & strAccCode & "</ACCCODE>"
                        strxml += "<MAPACCCODE>" & txtAccountCode.Text.Trim() & "</MAPACCCODE>"
                        strxml += "<DIVCODE>" & txtDivcode.Value & "</DIVCODE>"
                        strxml += "</ACCDET>"
                    End If
                End If
            Next
            ...
            myCommand = New SqlCommand("sp_addmod_acctmastnewmapping", SqlConn, sqlTrans)
            myCommand.CommandText = "sp_addmod_acctmastnewmapping"
            myCommand.CommandType = CommandType.StoredProcedure
            myCommand.Parameters.Add(New SqlParameter("@strxml", SqlDbType.NText)).Value = strxml
...
#Region "Public Function fnValidate() As Boolean"
    Public Function fnValidate() As Boolean
        Dim isRowChecked As Boolean = False
        If txtAccountCode.Text.Trim = "" Then
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Please select the account code.');", True)
            fnValidate = False
            Exit Function
        End If
        ...
        If isRowChecked = False Then
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Please check any one of row to map the account.');", True)
            fnValidate = False
            Exit Function
        End If
