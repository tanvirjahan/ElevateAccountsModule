''--------------------------------------------------------------------------------------------------------
'   Module Name    :    AcctGroupsNewSearch.aspx
...
#End Region
Partial Class AcctGroupsNewSearch
        ...
        Try
            ...
            If Page.IsPostBack = False Then
                ...
                Else
                    ...
                    objUser.CheckNewFormatUserRight(Session("dbconnectionName"), CType(Session("GlobalUserName"), String), CType(Session("Userpwd"), String),
                                                  CType(strappname, String), "AccountsModule\AcctGroupsNewSearch.aspx?appid=" + strappid, btnadd, btnEdit,
...
#Region "Fill Tree View"
    Private Sub PopulateRootLevel()
        ...
        strQry = "select parentID as [id],acctname,acctcode,(select count(*) FROM acctgroup_newBS   WHERE div_code='" & ViewState("divcode") & "' and childid=sc.parentID  and accttype=1) childnodecount FROM acctgroup_newBS sc where  div_code='" & ViewState("divcode") & "' and childid =0 and accttype=1" 'or  childid =0"
    ...
    Private Sub PopulateSubLevel(ByVal parentid As Integer, ByVal parentNode As TreeNode)
        ...
        strQry = " select parentID as [id],acctname,acctcode,(select count(*) FROM acctgroup_newBS WHERE div_code='" & ViewState("divcode") & "' and accttype=1 and  childid=sc.parentID) childnodecount FROM acctgroup_newBS sc where div_code='" & ViewState("divcode") & "' and  accttype=1 and childid=" & parentid
        Dim objCommand As New SqlCommand(strQry, mySqlConn)
...
#Region "Protected Sub btnadd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnadd.Click"
    Protected Sub btnadd_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnadd.Click
        ...
        intNewParent = objUtils.ExecuteQueryReturnSingleValuenew(Session("dbconnectionName"), "select max(isnull(parentid,0))+1 from acctgroup_newBS where div_code='" & ViewState("divcode") & "'")
        ...
        Else
            ...
            ord = IIf(strSplitcode(0) = "0", "", objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), "select acctorder from acctgroup_newBS  where div_code='" & ViewState("divcode") & "' and acctcode='" & strSplitcode(0) & "'"))

            'Session("State") = "New"
            'Response.Redirect("AcctGroup.aspx?ord=" & ord, False)
            Dim strpop As String = ""
            strpop = "window.open('AcctGroupNew.aspx?State=New&ord=" & ord & "&divid=" & ViewState("divcode") & "&ChildId=" & intSelPrantid & "&AccLevel=" & intselLeval & "&Parentid=" & intNewParent & "&Level=" & chkLevel(tnCollection, intselLeval, intNewParent) & "','AcctGroup','width=1000,height=620 left=20,top=20 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
...
#Region "Protected Sub btndelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btndelete.Click"
    Protected Sub btndelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btndelete.Click
        ...
        If objUtils.GetDBFieldFromLongnewdiv(Session("dbconnectionName"), "acctgroup_newBS", "parentid", "childid", intNewParent, "div_code", ViewState("divcode")) <> 0 Then
        ...
        'Response.Redirect("AcctGroup.aspx?ord=" & ord, False)
        Dim strpop As String = ""
        strpop = "window.open('AcctGroupNew.aspx?State=Delete&ord=" & ord & "&divid=" & ViewState("divcode") & "&ParentId=" & intNewParent & "&RefCode=" & strTemp.GetValue(0) & "','AcctGroup','width=1000,height=620 left=20,top=20 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
        ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
...
#Region "Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click"
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        If ddlAccountSearchType.Value = "Account Name" Then
            ...
            strQry = "select accrep.level1 ,accrep.level2,accrep.level3,accrep.level4,accrep.level5,accrep.level6,accrep.level7," &
                "accrep.acccode ,accrep.accname from accrep inner join acctgroup_newBS on accrep.acccode = acctgroup_newBS.acctcode and accrep.divcode=acctgroup_newBS.div_code " &
                " where acctgroup_newBS.div_code='" & ViewState("divcode") & "' and accttype =1 and accrep.accname like '%" & txtSerch.Text & "%'"
        ...
        ElseIf ddlAccountSearchType.Value = "Account Code" Then
            ...
            strQry = "select accrep.level1 ,accrep.level2,accrep.level3,accrep.level4,accrep.level5,accrep.level6,accrep.level7," &
                "accrep.acccode ,accrep.accname from accrep inner join acctgroup_newBS on accrep.acccode = acctgroup_newBS.acctcode and accrep.divcode=acctgroup_newBS.div_code " &
                " where acctgroup_newBS.div_code='" & ViewState("divcode") & "' and acctgroupaccttype =1 and accrep.acccode like '%" & txtSerch.Text & "%'"
            mySqlReader = objUtils.GetDataFromReadernew(Session("dbconnectionName"), strQry)
        ...
        Try
            ...
            strsqlqry = "select concat(char(30),case acctlevel when 1 then acctcode else '-' end) level1," &
                    ...
                    "case  when acctlevel>=5 then acctcode else '-' end  Detail,acctname Name" &
                        " from acctgroup_newBS where div_code='" & ViewState("divcode") & "' and accttype=1"
            ...
            objUtils.ExportToExcelnew(DS, Response, "Accounts Groups - New")

            con.Close()

        Catch ex As Exception
            objUtils.WritErrorLog("AcctGroupsNewSearch.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        End Try
