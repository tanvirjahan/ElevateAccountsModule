...
Partial Class AcctGroupNew
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        Catch ex As Exception
            ...
            objUtils.WritErrorLog("AcctGroupNew.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        End Try
    End Sub
...
#Region " Private Sub ShowRecord(ByVal RefCode As String)"
    Private Sub ShowRecord(ByVal RefCode As String)
        ...
        Try
            ...
            mySqlCmd = New SqlCommand("Select * from acctgroup_newBS Where div_code='" & ViewState("divcode") & "' and acctcode='" & RefCode & "'", mySqlConn)
...
#Region "Public Function fnValidate() As Boolean"
    Public Function fnValidate() As Boolean
        ...
        If ViewState("AcctgrpState") = "New" Then
            If objUtils.isDuplicatenew(Session("dbconnectionName"), "acctgroup_newBS", "acctcode", txtAccCode.Value.Trim) = 1 Then
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('This account code is already present.');", True)
                SetFocus(txtAccCode)
                fnValidate = False
                Exit Function
            End If
            If objUtils.isDuplicatenew(Session("dbconnectionName"), "acctgroup_newBS", "acctname", txtAccName.Value.Trim) = 1 Then
            ...
            End If
        ElseIf ViewState("AcctgrpState") = "Edit" Then
            'If objUtils.isDuplicateForModifynew(Session("dbconnectionName"),"acctgroup_newBS", "acctcode", "acctcode", txtAccCode.Value.Trim, CType(Session("RefCode"), String)) = 1 Then
            '    ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('This account code is already present.');", True)
            '    SetFocus(txtAccCode)
            '    fnValidate = False
            'End If
            If objUtils.isDuplicateForModifynew(Session("dbconnectionName"), "acctgroup_newBS", "acctcode", "acctname", txtAccName.Value.Trim, CType(Session("RefCode"), String)) = 1 Then
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('This account name is already present.');", True)
                SetFocus(txtAccCode)
                fnValidate = False
                Exit Function
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            If ViewState("AcctgrpState") = "New" Or ViewState("AcctgrpState") = "Edit" Then
                ...
                '//Save acctgroup_newBS
                If ViewState("AcctgrpState") = "New" Then
                    mySqlCmd = New SqlCommand("sp_add_acctnewgroup", mySqlConn, sqlTrans)
                    mySqlCmd.CommandText = "sp_add_acctnewgroup"
                ElseIf ViewState("AcctgrpState") = "Edit" Then
                    ' intParentId = CType(Session("RefCode"), String)
                    mySqlCmd = New SqlCommand("sp_mod_acctnewgroup", mySqlConn, sqlTrans)
                    mySqlCmd.CommandText = "sp_mod_acctnewgroup"
                End If
                ...
                If ViewState("AcctgrpState") = "New" Then
                    mySqlCmd = New SqlCommand("sp_add_accnewrep", mySqlConn, sqlTrans)
                    mySqlCmd.CommandText = "sp_add_accnewrep"
                ElseIf ViewState("AcctgrpState") = "Edit" Then
                    mySqlCmd = New SqlCommand("sp_mod_accnewrep", mySqlConn, sqlTrans)
                    mySqlCmd.CommandText = "sp_mod_accnewrep"
                End If
            ...
            ElseIf ViewState("AcctgrpState") = "Delete" Then
                ...
                mySqlCmd = New SqlCommand("sp_del_acctnewgroup", mySqlConn, sqlTrans)
                mySqlCmd.CommandText = "sp_del_acctnewgroup"
                mySqlCmd.CommandType = CommandType.StoredProcedure
...
#Region "Private Sub DisableControls()"
    Private Sub DisableControls()
        ...
        ElseIf ViewState("AcctgrpState") = "Edit" Then
            ...
            'ChildExt = objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"),"select 't' from acctgroup_newBS where accttype=1 and  childid in (select parentid from acctgroup_newBS where accttype=1 and  acctcode='" & CType(Session("RefCode"), String) & "' )")
            'If ChildExt = "t" Then
            '    ddlMajor.Disabled = True
            'Else
            '    ddlMajor.Disabled = False
