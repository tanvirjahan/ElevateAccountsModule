...
Partial Class AccountsModule_rptUpdatesupplierInvoices
            ...
            Else
                ...
                If Request.QueryString("InvoiceNo") <> "" Then
                    If strSelectionFormula <> "" Then
                        strReportTitle = strReportTitle & " ;Tran Id: " & Request.QueryString("InvoiceNo")
                        strSelectionFormula = strSelectionFormula & " and {supplierinvoice_header.tran_id} = '" & Request.QueryString("InvoiceNo") & "'"
                    Else
                        strReportTitle = "Tran Id: " & Request.QueryString("InvoiceNo")
                        strSelectionFormula = "{supplierinvoice_header.tran_id} = '" & Request.QueryString("InvoiceNo") & "'"
                    End If
                End If
                If Request.QueryString("Type") <> "[Select]" Then
                    If strSelectionFormula <> "" Then
                        strReportTitle = strReportTitle & " ;Type: " & Request.QueryString("Type")
                        strSelectionFormula = strSelectionFormula & " and {supplierinvoice_header.acc_type} = '" & Request.QueryString("Type") & "'"
                    Else
                        strReportTitle = "Type: " & Request.QueryString("Type")
                        strSelectionFormula = "{supplierinvoice_header.acc_type} ='" & Request.QueryString("Type") & "'"
                    End If
                End If
                If Request.QueryString("SupplierCode") <> "[Select]" Then
                    If strSelectionFormula <> "" Then
                        ...
                        strSelectionFormula = strSelectionFormula & " and {supplierinvoice_header.partycode} = '" & Request.QueryString("SupplierCode") & "'"
                    Else
                        strReportTitle = "Supplier Code: " & Request.QueryString("SupplierCode")
                        strSelectionFormula = "{supplierinvoice_header.partycode} = '" & Request.QueryString("SupplierCode") & "'"
                    End If
                End If


                Select Case CType(ViewState("Pageame"), String)
                    Case "UpdatesupplierInvoicesBrief"
                        ViewState.Add("strReportName", CType(Server.MapPath("~\Report\rptsupplierstatementmatchBrief.rpt"), String))
                        rptreportname = "Report - Purchase Invoice Brief"
                        Exit Select
                    Case "UpdatesupplierInvoicesDetail"
                        ViewState.Add("strReportName", CType(Server.MapPath("~\Report\rptsupplierstatementmatchdetail.rpt"), String))
