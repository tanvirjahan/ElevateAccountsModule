'------------================--------------=======================------------------================
'   Module Name    :    DefineControlAccounts-Suppliers.aspx
...
Partial Class DefineControlAccounts_Suppliers
        ...
        If Page.IsPostBack = False Then
            ...
            Try
                ...
                Else
                    ...
                    '                                   CType(Session("AppName"), String), "DefineControlAccounts-Suppliers.aspx", BtnAddNew, BtnExportToExcel, _
                    '                                   BtnPrint, gv_SearchResult, GridCol.Edit, GridCol.Delete, GridCol.View)

                End If
                Page.Title = Page.Title + "-" + "Define Control Accounts -Suppliers"

                objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlSuppleircode, "partycode", "partyname", "Select partycode,partyname from partymast where active=1 order by partycode", True)
                objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlSuppleirname, "partyname", "partycode", "Select partycode,partyname from partymast where active=1 order by partyname", True)
                ...
                If (Page.ClientScript.IsClientScriptIncludeRegistered(typ, "TADDScript.js")) = False Then
                    ...
                    ddlSuppleircode.Attributes.Add("onKeyDown", "TADD_OnKeyDown(this);")
                    ddlSuppleirname.Attributes.Add("onKeyDown", "TADD_OnKeyDown(this);")
            ...
            Catch ex As Exception
                ...
                objUtils.WritErrorLog("DefineControlAccounts-Suppliers.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
            End Try
                ...
                While mySqlReader.Read()
                    For Each gvRow In gv_SearchResult.Rows
                        If IsDBNull(mySqlReader("partycode")) = False Then
                            If CType(mySqlReader("partycode"), String) = gvRow.Cells(3).Text Then
...
#Region "  Private Function BuildCondition() As String"
    Private Function BuildCondition() As String
        Try
            strWhereCond = ""
            If ddlSuppleircode.Value <> "[Select]" Then
                If Trim(strWhereCond) = "" Then
                    strWhereCond = " partycode = '" & Trim(ddlSuppleircode.Items(ddlSuppleircode.SelectedIndex).Text) & "'"
                Else
                    strWhereCond = strWhereCond & " AND partycode = '" & Trim(ddlSuppleircode.Items(ddlSuppleircode.SelectedIndex).Text) & "'"
                End If
            End If
            If ddlSptypecode.Value <> "[Select]" Then
                If Trim(strWhereCond) = "" Then
                    strWhereCond = " partymast.sptypecode  = '" & Trim(ddlSptypecode.Items(ddlSptypecode.SelectedIndex).Text) & "'"
                Else
                    strWhereCond = strWhereCond & " AND partymast.sptypecode  = '" & Trim(ddlSptypecode.Items(ddlSptypecode.SelectedIndex).Text) & "'"
                End If
            End If

            If ddlCategorycode.Value <> "[Select]" Then
                If Trim(strWhereCond) = "" Then
                    strWhereCond = " catcode = '" & Trim(ddlCategorycode.Items(ddlCategorycode.SelectedIndex).Text) & "'"
                Else
                    strWhereCond = strWhereCond & " AND catcode = '" & Trim(ddlCategorycode.Items(ddlCategorycode.SelectedIndex).Text) & "'"
                End If
            End If
...
#Region "Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = 'ASC')"
    Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = "ASC")
        ...
        Try
            ...
            strSqlQry = " select partymast.*,sptypename from partymast inner join " & _
                        " sptypemast on partymast.sptypecode = sptypemast.sptypecode where partymast.active=1"
    ...
    Protected Sub btnExportToExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnExportToExcel.Click
        ...
        Try
            If gv_SearchResult.Rows.Count <> 0 Then
                ...
                strSqlQry = " SELECT  partymast.sptypecode as [SPType Code],sptypename as [SPType Name],partycode as [Supplier Code],partyname as [Supplier Name]," & _
                            ...
                            " FROM partymast inner join sptypemast on partymast.sptypecode = sptypemast.sptypecode " & _
                            " LEFT OUTER JOIN " & _
                            " acctmast ON partymast.controlacctcode = acctmast.acctcode LEFT OUTER JOIN " & _
                            " acctmast AS acctmast_1 ON partymast.accrualacctcode = acctmast_1.acctcode "


                If Trim(BuildCondition) <> "" Then
                    strSqlQry = strSqlQry & " Where " & BuildCondition() & " ORDER BY partycode"
                Else
    ...
    Public Function SaveGrid() As Boolean
        Try
            ...
            For Each grdRow In gv_SearchResult.Rows
                ...
                myCommand = New SqlCommand("sp_update_suppliers", SqlConn, sqlTrans)
                myCommand.CommandType = CommandType.StoredProcedure
                myCommand.Parameters.Add(New SqlParameter("@partycode", SqlDbType.VarChar, 20)).Value = CType(grdRow.Cells(3).Text.Trim, String)
            ...
            myCommand.CommandType = CommandType.StoredProcedure
            myCommand.Parameters.Add(New SqlParameter("@optionname", SqlDbType.VarChar, 100)).Value = "DefineControlAccounts_Suppliers"
            myCommand.Parameters.Add(New SqlParameter("@adddate", SqlDbType.DateTime)).Value = objdate.GetSystemDateTime(Session("dbconnectionName"))
...
#Region "Protected Sub BtnClear_Click(ByVal sender As Object, ByVal e As System.EventArgs)"
    Protected Sub BtnClear_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ddlSuppleircode.Value = "[Select]"
        ddlSuppleirname.Value = "[Select]"
...
#End Region
    ...
    Protected Sub btnhelp_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "window.open('../Help.aspx?hi=DefineControlAccounts-Suppliers','_blank','status=1,scrollbars=1,top=135,left=760,width=250,height=500');", True)
    End Sub
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        Try
            ...
            strpop = "window.open('rptReportNew.aspx?Pageame=DefineControlAccountsSuppliers&BackPageName=DefineControlAccounts-Suppliers.aspx&SPtypeCode=" & Trim(ddlSptypecode.Items(ddlSptypecode.SelectedIndex).Text) & "&SupCode=" & Trim(ddlSuppleircode.Items(ddlSuppleircode.SelectedIndex).Text) & "&SupCat=" & Trim(ddlCategorycode.Items(ddlCategorycode.SelectedIndex).Text) & "','DefCtrlacctSup','width=1000,height=620 left=20,top=20 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
...
#Region "SaveSelectedToGrid()"
    Public Function SaveSelectedToGrid() As Boolean
        Try
            ...
            myCommand = New SqlCommand("sp_copy_suppliers", SqlConn, sqlTrans)
            myCommand.CommandType = CommandType.StoredProcedure
            If ddlInccode.Value <> "[Select]" Then
                myCommand.Parameters.Add(New SqlParameter("@accrualacctcode", SqlDbType.VarChar, 20)).Value = CType(ddlInccode.Items(ddlInccode.SelectedIndex).Text, String)
            Else
