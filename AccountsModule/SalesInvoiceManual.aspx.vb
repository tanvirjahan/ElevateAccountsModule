...
Partial Class AccountsModule_SalesInvoiceManual
...
#End Region
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        If IsPostBack = False Then
            ...
            Try
                ...
                strSqlQry = " select acctcode controlacctcode,acctname from acctmast where isnull(controlyn,'')='Y' order by acctcode"
                ...
                strSqlQry = " select acctname,acctcode controlacctcode from acctmast where isnull(controlyn,'')='Y' order by acctname"
                ...
                '   CheckPostUnpostRight(CType(Session("GlobalUserName"), String), CType(Session("Userpwd"), String), CType("Accounts Module", String), "AccountsModule\SaleInvoiceSearch.aspx?tran_type=" & ViewState("CNDNOpen_type"))
                DisableControl()
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            btnAdjustBill.Attributes.Add("style", "display:none") 'Added by Archana on 25/03/2015 as AdjustBill should be hidden
            ...
            If IsPostBack = True Then
                ...
                Else
                    ...
                    'objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"),ddlCustomer, "code", "des", "select [Select]", False)
                    objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlCustomer, "code", "des", "select top 10  code,des from view_account   order by code", False)
    ...
    Private Sub FillControlACCODENames()
        ...
        If ddlType.Value = "C" Then
            sqlstr1 = " select acctcode controlacctcode,acctname from acctmast where  isnull(controlyn,'')='Y' and isnull(cust_supp,'')='C' order by acctcode"
            sqlstr2 = " select acctname,acctcode controlacctcode from acctmast where  isnull(controlyn,'')='Y' and isnull(cust_supp,'')='C' order by acctname"
        ElseIf ddlType.Value = "S" Then
            sqlstr1 = " select distinct partymast.controlacctcode  , acctmast.acctname  from acctmast ,partymast where   partymast.controlacctcode= acctmast.acctcode   and partymast.partycode='" + codeid + "' "
...
#Region "Public Sub FillGrids()"
    Public Sub FillGrids()
        ...
        ElseIf ViewState("DebitNoteState") <> "New" Then
            Dim lngCnt As Long
            lngCnt = objUtils.GetDBFieldFromStringnew(Session("dbconnectionName"), "trdpurchase_other", "count(tran_id)", "tran_id", CType(ViewState("DebitNoteRefCode"), String))
            If lngCnt > 1 Then
...
#Region "Private Sub FillGridDebitNote()"
    Private Sub FillGridDebitNote()
        ...
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objUtils.WritErrorLog("Package.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        End Try
    End Sub
#End Region
...
#Region "Private Sub ShowRecord(ByVal RefCode As String)"
    Private Sub ShowRecord(ByVal RefCode As String)
        Try
            ...
            myCommand = New SqlCommand("Select * from trdpurchase_master Where tran_id='" & RefCode & "' and tran_type='" & CType(ViewState("CNDNOpen_type"), String) & "' and div_id='" & CType(objUtils.GetDBFieldFromLongnew(Session("dbconnectionName"), "reservation_parameters", "option_selected", "param_id", 511), String) & "'", SqlConn)
            ...
            If mySqlReader.HasRows Then
                If mySqlReader.Read() = True Then
                    ...
                    If IsDBNull(mySqlReader("trd_gl_code")) = False Then
                        ...
                        txtConAccCode.Value = objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), "select acctname from acctmast where acctcode ='" & mySqlReader("trd_gl_code").ToString & "' ")
                    Else
...
#Region "Private Sub ShowGridDebitNote(ByVal RefCode As String)"
    Private Sub ShowGridDebitNote(ByVal RefCode As String)
        Try
            ...
            SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))           'connection open
            myCommand = New SqlCommand("Select * from trdpurchase_other Where tran_id='" & RefCode & "' and tran_type='" & CType(ViewState("CNDNOpen_type"), String) & "' and div_code='" & CType(objUtils.GetDBFieldFromLongnew(Session("dbconnectionName"), "reservation_parameters", "option_selected", "param_id", 511), String) & "'", SqlConn)
            mySqlReader = myCommand.ExecuteReader(CommandBehavior.CloseConnection)
            If mySqlReader.HasRows Then
                    ...
                    For Each gvRow In grdDebitNote.Rows
                        ...
                        If lblLineNo.Text = CType(mySqlReader("Sno"), String) - 1 Then
                            ...
                            If IsDBNull(mySqlReader("acc_code")) = False Then
                                ddlAccCode.Value = objUtils.GetDBFieldFromStringnew(Session("dbconnectionName"), "acctmast", "acctname", "acctcode", CType(mySqlReader("acc_code"), String))
        ...
        Try
            ...
            For Each gvRow In grdDebitNote.Rows
                ...
                If ddlAccCode.Value <> "[Select]" Or ddlCCCode.Value <> "[Select]" Or txtCValue.Text <> "" Or txtKWValue.Text <> "" Then
                    ...
                    'End If
                    If txtCValue.Text = "" Then
                        ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Enter Curr. Vlaue for sales invoice.');", True)
                        SetFocus(grdDebitNote)
                        ValidateGrids = False
                        Exit Function
                    End If
...
#End Region
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        Try
            Dim strdiv, strcostcentercode, plgrpcode As String
            ...
            If Page.IsValid = True Then
                ...
                End If
                If ddlType.Value <> "[Select]" Then
                    plgrpcode = objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), "select plgrpcode from agentmast(nolock) where agentcode='" & CType(ddlCustomer.Items(ddlCustomer.SelectedIndex).Text, String) & "'")
                End If
                ...
                If ViewState("DebitNoteState") = "New" Or ViewState("DebitNoteState") = "Edit" Or ViewState("DebitNoteState") = "Copy" Then
                    If ViewState("DebitNoteState") = "New" Or ViewState("DebitNoteState") = "Copy" Then
                        ...
                        txtDocNo.Text = ""
                        optionval = objUtils.GetAutoDocNo(CType(ViewState("CNDNOpen_type"), String), SqlConn, sqlTrans)
                    ...
                    End If
                    If Trim(plgrpcode) <> "" Then
                        myCommand.Parameters.Add(New SqlParameter("@plgrpcode", SqlDbType.VarChar, 20)).Value = plgrpcode
                    Else
                    ...
                    If chkPost.Checked = True Then
                        ...
                        End If
                        ctran.acc_ref1 = ""
                        ctran.acc_ref2 = txtReferenceNo.Text
                        ctran.acc_ref3 = ""
                        ctran.acc_ref4 = ""
                        cacc.addtran(ctran)
                        If CheckAdjustBill() = False Then
                            ...
                            If CType(ViewState("CNDNOpen_type"), String) = "INM" Then
                            ...
                            csubtran.acc_field1 = ""
                            csubtran.acc_field2 = txtReferenceNo.Text
                            csubtran.acc_field3 = ""
                            csubtran.acc_field4 = ""
                            csubtran.acc_field5 = ""
...
#End Region
    ...
    Protected Sub btnAdjustBill_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ScriptStr = "<script language=""javascript"">var win=window.open('../AccountsModule/ReceiptsAdjustBills.aspx?" + pass + "','ReceiptsAdjustBills','toolbar=0,scrollbars=yes,resizable=yes,titlebar=0,menubar=0,locationbar=0,modal=1,statusbar=1,fullscreen=yes');</script>"
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            backPname = "~\AccountsModule\SaleInvoiceManualSearch.aspx?tran_type=" & CType(ViewState("CNDNOpen_type"), String)
            ...
            'End If
            'If Trim(strSelectionFormula) = "" Then
            '    strSelectionFormula = " {trdpurchase_master.tran_type} = '" & ViewState("CNDNOpen_type") & "' " & _
            ...
            'Else
            '    strSelectionFormula = strSelectionFormula & " AND {trdpurchase_master.tran_type} = '" & ViewState("CNDNOpen_type") & "'" & _
            '    " and  {trdpurchase_master.div_id} = '" & CType(objUtils.GetDBFieldFromLongnew(Session("dbconnectionName"),"reservation_parameters", "option_selected", "param_id", 511), String) & "'"
            'End If
            ...
            'Dim ScriptStr As String
            'ScriptStr = "<script language=""javascript"">var win=window.open('../PriceListModule/PrintDocNew.aspx','printdoc','toolbar=0,scrollbars=yes,resizable=yes,titlebar=0,menubar=0,locationbar=0,modal=1,statusbar=1');</script>"
            'ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", ScriptStr, False)
            'Dim ScriptStr As String
            'ScriptStr = "<script language=""javascript"">var win=window.open('../PriceListModule/PrintDocNew.aspx?Pageame=DebitNoteDoc&BackPageName=" & backPname & "&TranId=" & txtDocNo.Text & "&TranType=" & ViewState("CNDNOpen_type") & "','printdoc','toolbar=0,scrollbars=yes,resizable=yes,titlebar=0,menubar=0,locationbar=0,modal=1,statusbar=1');</script>"
            'ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", ScriptStr, False)


            Dim ScriptStr As String
            ScriptStr = "<script language=""javascript"">var win=window.open('../Reservation/rptConfirmaton.aspx?reqid=" & CType(txtDocNo.Text, String) & "&typ=InvoiceManual&GrpM=1','printdoc','toolbar=0,scrollbars=yes,resizable=yes,titlebar=0,menubar=0,locationbar=0,modal=1,statusbar=1');</script>"
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", ScriptStr, False)
            ...
            'strpop = "window.open('../Reservation/rptConfirmaton.aspx?reqid=" + CType(txtDocNo.Text, String) + "&typ=InvoiceManual&Grp=0 PopUpGuestDetails','width=1010,height=650 left=0,top=0 scrollbars=yes,resizable=yes,status=yes');"
            'ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)


            'Dim Myds As New DataSet
            'strSqlQry = "EXEC sp_repdoc_debitenote '" & CType(objUtils.GetDBFieldFromLongnew(Session("dbconnectionName"),"reservation_parameters", "option_selected", "param_id", 511), String) & "','" & txtDocNo.Text.Trim & "','" & Session("CNDNOpen_type") & "' "

            'SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
            'Dim da As SqlDataAdapter
            'da = New SqlDataAdapter(strSqlQry, SqlConn)
            'da.Fill(Myds)
            'Session.Add("PrinDocDS", Myds)
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
