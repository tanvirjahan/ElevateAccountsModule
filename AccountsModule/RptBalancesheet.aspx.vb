...
#End Region
    ...
    Protected Sub btnExcelReport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExcelReport.Click
        Try
            ...
            Dim strpop As String = ""
            strpop = "window.open('TransactionReports.aspx?printId=BalanceSheet&reportsType=excel&pagetype=B&todate=" & strtodate & "&newformat=" & IIf(chknew.Checked = True, 1, 0) & "&divid=" & divcode & "&level=" & strlevel & "','RepBalSheet');"
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            'End If
        Catch ex As Exception
            If mySqlConn.State = ConnectionState.Open Then
