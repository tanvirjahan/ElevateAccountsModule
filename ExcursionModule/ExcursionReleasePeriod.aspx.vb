...
Partial Class ExcursionModule_ExcursionReleasePeriod
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        Try
            If Page.IsPostBack = False Then
                ViewState.Add("ExcursionReleasePeriodState", Request.QueryString("State"))
                ViewState.Add("ExcursionReleasePeriodRefCode", Request.QueryString("RefCode"))
                ...
                If ViewState("ExcursionReleasePeriodState") = "New" Then
                    'SetFocus(ddlSpTypeCD)
                    lblHeading.Text = "Add New Excursion Release Period"
                ...
                ElseIf ViewState("ExcursionReleasePeriodState") = "View" Then
                    lblHeading.Text = "View Excursion Release Period"
                    ShowRecord(ViewState("ExcursionReleasePeriodRefCode"))
                    ShowAllotmentnew_Details(ViewState("ExcursionReleasePeriodRefCode"))
                    ShowDates(ViewState("ExcursionReleasePeriodRefCode"))
                    FillGridMarket("plgrpcode")
                    ShowGridMarket(ViewState("ExcursionReleasePeriodRefCode"))

                    DisableAllControls()
                    SetFocus(btnExit)
                ElseIf ViewState("ExcursionReleasePeriodState") = "Edit" Then
                    lblHeading.Text = "Edit Excursion Release Period"
                    ShowRecord(ViewState("ExcursionReleasePeriodRefCode"))
                    ShowAllotmentnew_Details(ViewState("ExcursionReleasePeriodRefCode"))
                    ShowDates(ViewState("ExcursionReleasePeriodRefCode"))
                    FillGridMarket("plgrpcode")
                    ...
                    SetFocus(btnSave)
                ElseIf ViewState("ExcursionReleasePeriodState") = "Delete" Then
                    lblHeading.Text = "Delete Excursion Release Period"
                    ShowRecord(ViewState("ExcursionReleasePeriodRefCode"))
                    ShowAllotmentnew_Details(ViewState("ExcursionReleasePeriodRefCode"))
                    ShowDates(ViewState("ExcursionReleasePeriodRefCode"))
                    FillGridMarket("plgrpcode")
            ...
            End If
            btnSave.Attributes.Add("onclick", "return FormValidation('" + Request.QueryString("State") + "')")


        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Error Description: - " & ex.Message & "');", True)
            objUtils.WritErrorLog("ExcursionReleasePeriod.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        End Try
...
#Region "Private Sub ShowRecord(ByVal RefCode As String)"
    Private Sub ShowRecord(ByVal RefCode As String)
        Try
            strSqlQry = "select * from excallotmentnew_header where allotid='" & RefCode & "'"
            SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
            SqlCmd = New SqlCommand(strSqlQry, SqlConn)
            mySqlReader = SqlCmd.ExecuteReader
            If mySqlReader.Read = True Then
                If IsDBNull(mySqlReader("allotid")) = False Then
                    txtAllotmentID.Value = mySqlReader("allotid")
                End If
...
#Region "Private Sub ShowAllotmentnew_Details(ByVal RefCode As String)"
    Private Sub ShowAllotmentnew_Details(ByVal RefCode As String)
        ...
        Try
            ...
            SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
            strSqlQry = " SELECT 0 as rowid,alineno as [lineno], isnull(plgrpcode,'')plgrpcode, convert(varchar(10),allotdate,103)as allotdate,LEFT(DATENAME(dw, allotdate), 3) AS DayofWeek,stopsale,cancdays as ReleasePeriod" & _
                       "   FROM excallotmentnew_detail where allotid='" & RefCode & "'"
...
#Region " Private Sub ShowDates(ByVal RefCode As String)"
    ...
    Private Sub ShowDates(ByVal RefCode As String)
        Try
            ...
            SqlCmd = New SqlCommand("Select * from excallotmentnew_dates Where allotid='" & RefCode & "'", SqlConn)
            mySqlReader = SqlCmd.ExecuteReader(CommandBehavior.CloseConnection)
            Dim ct As Integer
            If mySqlReader.HasRows Then
                ct = objUtils.ExecuteQueryReturnSingleValuenew(Session("dbconnectionName"), "Select count(*) from excallotmentnew_dates Where allotid='" & RefCode & "'")
...
#Region " Private Sub ShowGridMarket(ByVal RefCode As String)"
    ...
    Private Sub ShowGridMarket(ByVal RefCode As String)
        Try
            ...
            SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))           'connection open
            SqlCmd = New SqlCommand("Select * from excallotmentnew_genmkt   Where allotid='" & RefCode & "'", SqlConn)
            mySqlReader = SqlCmd.ExecuteReader(CommandBehavior.CloseConnection)
            If mySqlReader.HasRows Then
                While mySqlReader.Read()
                    For Each gvRow In gv_Mkt.Rows
                        ...
                        If IsDBNull(mySqlReader("mktcode")) = False Then
                            If gvRow.Cells(1).Text.Trim = mySqlReader("mktcode") Then
                                chk.Checked = True
                            End If
                        End If
                    Next
...
#Region "Protected Sub btnGenerate_Click(ByVal sender As Object, ByVal e As System.EventArgs)"
    Protected Sub btnGenerate_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnGenerate.Click 'Handles btnGenerate.Click
        ...
        Try
            ...
            'If ddlAllotmentType.Value = "[Select]" Then
            '    allotpye = ""
            'ElseIf ddlAllotmentType.Value = "Allotment" Then
            '    allotpye = "AL"
            'ElseIf ddlAllotmentType.Value = "Back to Back" Then
            '    allotpye = "BL"
            'End If
                ...
                If chkmkt.Checked = True Then
                    ...
                    For Each GvRow In grdDates.Rows
                        ...
                        If frmdate.Text <> "" And todate.Text <> "" Then
                            ...
                            strSqlQry = "EXEC sp_excgen_allotments '" & othgrpcode & "','" & othtypcode & "','" & CType(Format(CType(frmdate.Text, Date), "yyyy/MM/dd"), String) & "','" & CType(Format(CType(todate.Text, Date), "yyyy/MM/dd"), String) & "','" & market.Trim & "','" & includemarkets.Trim & "'"

                            If cnt = 0 Then
                                dt.Columns.Add(New DataColumn("lineno", GetType(String)))
                                dt.Columns.Add(New DataColumn("rowid", GetType(String)))
                                dt.Columns.Add(New DataColumn("plgrpcode", GetType(String)))
                                dt.Columns.Add(New DataColumn("allotdate", GetType(String)))
                                dt.Columns.Add(New DataColumn("DayofWeek", GetType(String)))
                                ...
                                While mySqlReader.Read = True
                                    ...
                                    dr(2) = mySqlReader("plgrpcode")
                                    dr(3) = Format("U", mySqlReader("allotdate")) 'ObjDate.ConvertDateFromDatabaseToTextBoxFormat(mySqlReader("allotdate"))
                                    dr(4) = mySqlReader("DayofWeek")
                                    dr(5) = mySqlReader("ReleasePeriod")
                                    dr(6) = mySqlReader("stopsale")
                                    dt.Rows.Add(dr)
                                End While
        ...
        Try
            If Page.IsValid = True Then
                ...
                If Request.QueryString("State") = "New" Then
                    ...
                    optionval = objUtils.GetAutoDocNo("EXCRELP", mySqlConn, sqlTrans)
                    txtAllotmentID.Value = optionval.Trim
                    SqlCmd = New SqlCommand("sp_add_excallotmentnew_header", mySqlConn, sqlTrans)
                    SqlCmd.CommandType = CommandType.StoredProcedure

                    SqlCmd.Parameters.Add(New SqlParameter("@allotid", SqlDbType.VarChar, 30)).Value = CType(txtAllotmentID.Value.Trim, String)
                    ...
                    If chkapprove.Checked = True Then
                        SqlCmd.Parameters.Add(New SqlParameter("@approve", SqlDbType.Int, 9)).Value = 1
                    Else
                        SqlCmd.Parameters.Add(New SqlParameter("@approve", SqlDbType.Int, 9)).Value = 0
                    End If
                    ...
                    For Each GvRow In gv_allotment.Rows
                        ...
                        SqlCmd = New SqlCommand("sp_add_excallotmentnew_detail", mySqlConn, sqlTrans)
                        SqlCmd.CommandType = CommandType.StoredProcedure
                        SqlCmd.Parameters.Add(New SqlParameter("@allotid", SqlDbType.VarChar, 20)).Value = CType(txtAllotmentID.Value.Trim, String)
                        ...
                        If Trim(GvRow.Cells(2).Text) = "" Then
                            SqlCmd.Parameters.Add(New SqlParameter("@allotdate", SqlDbType.DateTime)).Value = DBNull.Value
                        Else
                            SqlCmd.Parameters.Add(New SqlParameter("@allotdate", SqlDbType.DateTime)).Value = Format(CType(GvRow.Cells(3).Text, Date), "yyyy/MM/dd")
                        End If

                        If Trim(GvRow.Cells(2).Text) = "" Then
                            SqlCmd.Parameters.Add(New SqlParameter("@allotdatec", SqlDbType.DateTime)).Value = DBNull.Value
                        Else
                            SqlCmd.Parameters.Add(New SqlParameter("@allotdatec", SqlDbType.DateTime)).Value = Format(CType(GvRow.Cells(3).Text, Date), "yyyy/MM/dd")
                        End If
                        SqlCmd.Parameters.Add(New SqlParameter("@cancdays", SqlDbType.Int)).Value = txtRelease.Text
                        SqlCmd.Parameters.Add(New SqlParameter("@rowid", SqlDbType.BigInt)).Value = Val(lblrowid.Text)
                        lblstopsale = GvRow.FindControl("lblstopsale")
                        If lblstopsale.Text <> "" Then
                            SqlCmd.Parameters.Add(New SqlParameter("@stopsale", SqlDbType.Int)).Value = lblstopsale.Text
                        Else
                            SqlCmd.Parameters.Add(New SqlParameter("@stopsale", SqlDbType.Int)).Value = lblstopsale.Text
                        End If
                        SqlCmd.Parameters.Add(New SqlParameter("@remarks", SqlDbType.VarChar, 500)).Value = CType(txtRemark.Value.Trim, String)
                    ...
                    For Each GvRow In gv_allotment.Rows
                        ...
                        SqlCmd = New SqlCommand("sp_update_releaseperiod_exc", mySqlConn, sqlTrans)
                        SqlCmd.CommandType = CommandType.StoredProcedure
                        lbl = GvRow.FindControl("lblrowid")
                        txtRelease = GvRow.FindControl("txtRelease")
                        lblstopsale = GvRow.FindControl("lblstopsale")
                        SqlCmd.Parameters.Add(New SqlParameter("@rowid", SqlDbType.BigInt)).Value = Val(lbl.Text)
                        SqlCmd.Parameters.Add(New SqlParameter("@allotdate", SqlDbType.VarChar, 10)).Value = Format(CType(GvRow.Cells(3).Text, Date), "yyyy/MM/dd")
                        ...
                        If CType(plgrpcode, String) = "[Select]" Then
                            SqlCmd.Parameters.Add(New SqlParameter("@plgrpcode", SqlDbType.VarChar, 20)).Value = DBNull.Value
                        Else
                            SqlCmd.Parameters.Add(New SqlParameter("@plgrpcode", SqlDbType.VarChar, 20)).Value = CType(GvRow.Cells(2).Text, String)
                        End If
                        SqlCmd.Parameters.Add(New SqlParameter("@cancdays", SqlDbType.Int)).Value = txtRelease.Text
                        SqlCmd.Parameters.Add(New SqlParameter("@allotdatec", SqlDbType.VarChar, 10)).Value = Format(CType(GvRow.Cells(3).Text, Date), "yyyy/MM/dd")
                        If lblstopsale.Text <> "" Then
                            SqlCmd.Parameters.Add(New SqlParameter("@stopsale", SqlDbType.Int)).Value = CType(lblstopsale.Text, Integer)
                        Else
                            SqlCmd.Parameters.Add(New SqlParameter("@stopsale", SqlDbType.Int)).Value = CType(lblstopsale.Text, Integer)
                        End If
                        SqlCmd.Parameters.Add(New SqlParameter("@remarks", SqlDbType.VarChar, 200)).Value = CType(txtRemark.Value.Trim, String)
                    ...
                    SqlCmd = New SqlCommand("sp_del_excallotmentnew_dates", mySqlConn, sqlTrans)
                    SqlCmd.CommandType = CommandType.StoredProcedure
                    SqlCmd.Parameters.Add(New SqlParameter("@allotid", SqlDbType.VarChar, 20)).Value = CType(txtAllotmentID.Value.Trim, String)
                    ...
                    For Each GvRow In grdDates.Rows
                        ...
                        If frmdt.Text <> "" And todt.Text <> "" Then
                            SqlCmd = New SqlCommand("sp_add_excallotmentnew_dates", mySqlConn, sqlTrans)
                            SqlCmd.CommandType = CommandType.StoredProcedure
                            SqlCmd.Parameters.Add(New SqlParameter("@allotid", SqlDbType.VarChar, 20)).Value = CType(txtAllotmentID.Value.Trim, String)
                            ...
                            SqlCmd.Parameters.Add(New SqlParameter("@cancdays", SqlDbType.Int)).Value = 0
                    ...
                    SqlCmd = New SqlCommand("sp_del_excallotmentnew_genmkt", mySqlConn, sqlTrans)
                    SqlCmd.CommandType = CommandType.StoredProcedure
                    SqlCmd.Parameters.Add(New SqlParameter("@allotid", SqlDbType.VarChar, 20)).Value = CType(txtAllotmentID.Value.Trim, String)
                    SqlCmd.ExecuteNonQuery()
                    ...
                    For Each GvRow In gv_Mkt.Rows
                        ...
                        If chkSel.Checked = True Then
                            SqlCmd = New SqlCommand("sp_add_excallotmentnew_genmkt", mySqlConn, sqlTrans)
                            SqlCmd.CommandType = CommandType.StoredProcedure
                            SqlCmd.Parameters.Add(New SqlParameter("@allotid", SqlDbType.VarChar, 20)).Value = CType(txtAllotmentID.Value.Trim, String)
                            SqlCmd.Parameters.Add(New SqlParameter("@mktcode", SqlDbType.VarChar, 20)).Value = CType(GvRow.Cells(1).Text.Trim, String)
                            SqlCmd.ExecuteNonQuery()
                        End If
                    Next
                ...
                ElseIf Request.QueryString("State") = "Edit" Then ''''************************************************************************
                    ...
                    SqlCmd = New SqlCommand("sp_Edit_excallotmentnew_header", mySqlConn, sqlTrans)
                    SqlCmd.CommandType = CommandType.StoredProcedure
                    ...
                    SqlCmd = New SqlCommand("sp_delete_excallotmentnew_detail", mySqlConn, sqlTrans)
                    SqlCmd.CommandType = CommandType.StoredProcedure
                    SqlCmd.Parameters.Add(New SqlParameter("@allotid", SqlDbType.VarChar, 20)).Value = CType(txtAllotmentID.Value.Trim, String)
                    SqlCmd.ExecuteNonQuery()
                ...
                ElseIf Request.QueryString("State") = "Delete" Then
                    ...
                    SqlCmd = New SqlCommand("sp_del_excallotmentnew_all", mySqlConn, sqlTrans)
                    SqlCmd.CommandType = CommandType.StoredProcedure
                    SqlCmd.Parameters.Add(New SqlParameter("@allotid", SqlDbType.VarChar, 20)).Value = CType(txtAllotmentID.Value.Trim, String)
                    SqlCmd.ExecuteNonQuery()
                    ...
                    For Each GvRow In gv_allotment.Rows
                        ...
                        SqlCmd.CommandType = CommandType.StoredProcedure
                        SqlCmd.Parameters.Add(New SqlParameter("@rowid", SqlDbType.VarChar, 20)).Value = Val(lbl.Text)
...
#End Region
    ...
    Protected Sub gv_allotment_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gv_allotment.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            txtRelease = e.Row.FindControl("txtRelease")
            lblstopsale = e.Row.FindControl("lblstopsale")
            NumbersForTextbox(txtRelease)

            If lblstopsale.Text = "1" Then
...
#Region "Protected Sub btnFill_Click(ByVal sender As Object, ByVal e As System.EventArgs)"
    Protected Sub btnFill_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Dim GvRow As GridViewRow
        Dim txt As TextBox
        Dim txtstop As TextBox
        Dim txtavl As TextBox
        Dim txtredu As TextBox
        Dim txtnowavl As TextBox
        Dim stopsalerow As Integer
        Try
            If txtFillValues.Value <> "" Then
                If IsNumeric(txtFillValues.Value) = False Then
                    ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Please enter valid numeric value.');", True)
                    ' ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "FocusScr", "WebForm_AutoFocus('" + txtFillValues.ClientID + "');", True)
                    ' imgicon.Style("visibility") = "hidden"
                    Exit Sub
                End If
            Else
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Please enter numeric value.');", True)
                ' ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "FocusScr", "WebForm_AutoFocus('" + txtFillValues.ClientID + "');", True)
                ' imgicon.Style("visibility") = "hidden"
                Exit Sub
            End If
            ...
            For Each GvRow In gv_allotment.Rows
                'txtstop = GvRow.FindControl("txtbufferrooms")
                'If txtstop.Text = "" Or txtstop.Text = "0" Or txtstop.Text = Nothing Then
                '    stopsalerow = 0
                'Else
                '    stopsalerow = 1
                '    GvRow.BackColor = System.Drawing.Color.FromName("#ff3300")
                'End If
                stopsalerow = 0
                If stopsalerow = 0 Then
                    If ddlFillValues.Value = "Release Period" Then
                        txt = GvRow.FindControl("txtRelease")
                        If txt.ReadOnly = False Then
                            txt.Text = txtFillValues.Value
                        End If
                    ElseIf ddlFillValues.Value = "Buffer Rooms" Then
                        'txt = GvRow.FindControl("txtbufferrooms")
                        'txt.Text = txtFillValues.Value
                        'txt.Enabled = False
                    ElseIf ddlFillValues.Value = "Increase" Then
                        'txt = GvRow.FindControl("txtIncrease")
                        'txtavl = GvRow.FindControl("txtAvailable")
                        'txtredu = GvRow.FindControl("txtReduce")
                        'txtnowavl = GvRow.FindControl("txtnowavailable")
                        'If Val(txtavl.Text) + Val(txtFillValues.Value) - Val(txtredu.Text) < 0 Then
                        '    ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('This is not valid increase value.');", True)
                        '    ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "FocusScr", "WebForm_AutoFocus('" + txtrelease.ClientID + "');", True)
                        'Else
                        '    txt.Text = txtFillValues.Value
                        '    txtnowavl.Text = Val(txtavl.Text) + Val(txt.Text) - Val(txtredu.Text)
                        'End If
                    ElseIf ddlFillValues.Value = "Minnights" Then
                        'txt = GvRow.FindControl("txtMinnights")
                        'txt.Text = txtFillValues.Value
                    ElseIf ddlFillValues.Value = "Overbook %" Then
                        'txt = GvRow.FindControl("txtOverbookPerc")
                        'txt.Text = txtFillValues.Value
                    ElseIf ddlFillValues.Value = "No of days Prior to Overbook" Then
                        'txt = GvRow.FindControl("txtoverbookdays")
                        'txt.Text = txtFillValues.Value
                    End If
                End If
            Next
        ...
        Catch ex As Exception
            ' imgicon.Style("visibility") = "hidden"
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Error Description: - " & ex.Message & "');", True)
            objUtils.WritErrorLog("MainAllotments.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        End Try
    End Sub
#End Region

    Private Function fngenclick()
        Dim othgrpcode As String
        Dim othtypcode As String
        Dim frmdate As TextBox
        Dim todate As TextBox
