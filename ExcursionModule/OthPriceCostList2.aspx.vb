        ...
        Try
            ...
            If IsPostBack = False Then
                ...
                If Session("OthPListFilter") <> "OTH" Then
                    SPType = "select partycode,partyname from partymast where   active=1 and partymast.sptypecode <>(select option_selected from reservation_parameters where param_id ='458')"
                Else
                    SPType = "select partycode,partyname from partymast where   active=1 and partymast.sptypecode <>(select option_selected from reservation_parameters where param_id ='458')"

                End If
                'objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlSPType, "partycode", "partyname", "select partycode,partyname from partymast where active=1 and partymast.sptypecode <>(select option_selected from reservation_parameters where param_id ='458')", True)
                'objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlSPTypeName, "partyname", "partycode", "select partyname,partycode from partymast where active=1 and partymast.sptypecode <>(select option_selected from reservation_parameters where param_id ='458')", True)
                ...
                objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlSPTypeName, "partyname", "partycode", "select partycode,partyname from partymast where active=1 and partymast.sptypecode not in (select option_selected from reservation_parameters where param_id IN('458','500')) order by partycode", True)


                objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlGroupCd, "othgrpcode", "othgrpname", "select othgrpcode , othgrpname from othgrpmast  where active=1 and othmaingrpcode in (select option_selected from reservation_parameters where param_id  in ('1021','1105')) order by othgrpcode", True)
                objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlGroupNm, "othgrpname", "othgrpcode", "select othgrpcode , othgrpname from othgrpmast  where active=1 and othmaingrpcode in (select option_selected from reservation_parameters where param_id in ('1021','1105')) order by othgrpname", True)
        ...
        If IsPostBack = False Then
            Try
                ...
                If ViewState("OthpricelistState2") = "View" Then
                    lblHeading.Text = "View Excursion Cost Price List"
                    ShowRecord(ViewState("OthpricelistRefCode2"))
                    ShowDates(CType(ViewState("OthpricelistRefCode2"), String))
                End If
                If ViewState("OthpricelistState2") = "Edit" Then
                    lblHeading.Text = "Edit Excursion Cost Price List"

                    ShowDates(CType(ViewState("OthpricelistRefCode2"), String))
                End If
                If ViewState("OthpricelistState2") = "Delete" Then
                    lblHeading.Text = "Delete Excursion Cost Price List"
            ...
            If mySqlReader.HasRows Then
                If mySqlReader.Read() = True Then
                    ...
                    If IsDBNull(mySqlReader("gpcode")) = False Then
                        ...
                        ddlGroupNm.Value = objUtils.GetDBFieldFromStringnew(Session("dbconnectionName"), "exccplist_header", "gpcode", "eplistcode", mySqlReader("eplistcode"))
...
#Region " Private Sub ShowDates(ByVal RefCode As String)"
    Private Sub ShowDates(ByVal RefCode As String)
        Try
            ...
            mySqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))           'connection open
            mySqlCmd = New SqlCommand("Select * from exccplist_dates Where eplistcode='" & RefCode & "'", mySqlConn)
            mySqlReader = mySqlCmd.ExecuteReader(CommandBehavior.CloseConnection)
            If mySqlReader.HasRows Then
...
#Region "Private Sub ShowDynamicGrid()"
    ...
    Private Sub ShowDynamicGrid(ByVal RefCode As String)
        Try
            ...
            StrQry = "select distinct exccplist_detail.exccode from exccplist_detail where exccplist_detail.eplistcode = '" & RefCode & "'"
            ...
            For Each GVROW In gv_SearchResult.Rows
                ...
                StrQryTemp = "select exccplist_detail.eplistcode  ,exccplist_detail.exccode,exccplist_detail.othgrpcode,exccplist_detail.adttype, " & _
  " exccplist_detail.price,exccplist_detail.pkg from  exccplist_detail  " & _
  " where   " & _
  " exccplist_detail.eplistcode= '" & RefCode & "' and exccode = '" & Replace(GVROW.Cells(2).Text, "amp;", "") & "'"
...
#Region " Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs)"
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs)
            ...
            If Page.IsValid = True Then
                If ViewState("OthpricelistState2") = "New" Or ViewState("OthpricelistState2") = "Edit" Or ViewState("OthpricelistState2") = "Copy" Then
                    ...
                    If ViewState("OthpricelistState2") = "New" Or ViewState("OthpricelistState2") = "Copy" Then
                        ...
                        mySqlCmd = New SqlCommand("sp_add_exccPListh", mySqlConn, sqlTrans)
                    ElseIf ViewState("OthpricelistState2") = "Edit" Then
                        mySqlCmd = New SqlCommand("sp_mod_exccplisthnew", mySqlConn, sqlTrans)
                    End If
                    ...
                    mySqlCmd.Parameters.Add(New SqlParameter("@partycode", SqlDbType.VarChar, 20)).Value = CType(ddlSPType.Items(ddlSPType.SelectedIndex).Text.Trim, String)
                    ...
                    mySqlCmd = New SqlCommand("sp_del_exccplisth_dates", mySqlConn, sqlTrans)
                    mySqlCmd.CommandType = CommandType.StoredProcedure
                    ...
                    For Each GvRow In grdDates.Rows
                        ...
                        If dpFDate.Text <> "" And dpTDate.Text <> "" Then
                            mySqlCmd = New SqlCommand("sp_add_exccplisth_dates", mySqlConn, sqlTrans)
                            mySqlCmd.CommandType = CommandType.StoredProcedure
                            mySqlCmd.Parameters.Add(New SqlParameter("@eplistcode", SqlDbType.VarChar, 20)).Value = CType(txtPlcCode.Value.Trim, String)
                    ...
                    mySqlCmd = New SqlCommand("sp_upd_min_max_dates_exc_cost", mySqlConn, sqlTrans)
                    mySqlCmd.CommandType = CommandType.StoredProcedure
                    mySqlCmd.Parameters.Add(New SqlParameter("@eplistcode", SqlDbType.VarChar, 20)).Value = CType(txtPlcCode.Value.Trim, String)
                    mySqlCmd.ExecuteNonQuery()
                    ...
                    mySqlCmd = New SqlCommand("sp_del_exccplistCostd", mySqlConn, sqlTrans)
                    mySqlCmd.CommandType = CommandType.StoredProcedure
                    mySqlCmd.Parameters.Add(New SqlParameter("@eplistcode", SqlDbType.VarChar, 20)).Value = CType(txtPlcCode.Value.Trim, String)
                    mySqlCmd.ExecuteNonQuery()
                        ...
                        If n = 0 Then
                            For j = 0 To cnt - 5
                                ...
                                ElseIf txt.Text <> "" Then
                                    mySqlCmd = New SqlCommand("sp_add_exccplistCostd", mySqlConn, sqlTrans)
                                    mySqlCmd.CommandType = CommandType.StoredProcedure
                                    mySqlCmd.Parameters.Add(New SqlParameter("@eplistcode", SqlDbType.VarChar, 20)).Value = CType(txtPlcCode.Value.Trim, String)
                                    mySqlCmd.Parameters.Add(New SqlParameter("@exccode", SqlDbType.VarChar, 20)).Value = CType(Replace(GvRow.Cells(2).Text, "amp;", ""), String)
                ...
                ElseIf ViewState("OthpricelistState2") = "Delete" Then
                    ...
                    mySqlCmd = New SqlCommand("sp_del_exccplisth", mySqlConn, sqlTrans)
                    mySqlCmd.CommandType = CommandType.StoredProcedure
                    mySqlCmd.Parameters.Add(New SqlParameter("@eplistcode", SqlDbType.VarChar, 20)).Value = CType(txtPlcCode.Value.Trim, String)
                    mySqlCmd.ExecuteNonQuery()
        ...
        Catch ex As Exception
            ...
            End If
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objUtils.WritErrorLog("OthPriceCostList2.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        End Try
    ...
    Public Sub fillDategrd(ByVal grd As GridView, Optional ByVal blnload As Boolean = False, Optional ByVal count As Integer = 1)
        ...
        Else
            If ViewState("OthpricelistRefCode2") <> Nothing Then
                mySqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
                strSqlQry = "select count(*) from exccplist_dates where eplistcode='" + ViewState("OthpricelistRefCode2") + "'"
                mySqlCmd = New SqlCommand(strSqlQry, mySqlConn)
    ...
    Private Function mycheckdates() As Boolean
        ...
        Try
            ...
            For Each GvRow In grdDates.Rows
                ...
                dpTDate = GvRow.FindControl("txtToDate")
                If dpFDate.Text <> "" And dpTDate.Text <> "" Then
                    mySqlCmd = New SqlCommand("sp_chk_dates_exc_cost", mySqlConn, sqlTrans)
