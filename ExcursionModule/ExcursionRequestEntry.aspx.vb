...
Partial Class ExcursionModule_ExcursionRequestEntry
        ...
        Try
            If Page.IsPostBack = False Then
                ...
                If CType(Session("ExcursionRequestState"), String) = "New" Then
                    ...
                    strSql = "select lastno+1 from docgen(nolock) where optionname='EX'"
                    hdnExcursionID.Value = "EX/" + objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), strSql)

                    'This is added by Riswan - After saving from sub entry page with multo cost, one record has saved to temp table. if we exit from request entry page with out main save, when we add a new record the same excursion id is generated and that is already present in temp table. when we add excursion, this new record is automatically considered as multo cost data. thats why while adding a new record, delete from the temp table by the respective excursion id.

                    'Dim mdelQry As String = "DELETE FROM Excursions_Cost_detail_temp Where excid='" & hdnExcursionID.Value & "'"
                    'SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
                    'SqlCmd = New SqlCommand(mdelQry, SqlConn)
                    ...
                    hdnExcursionID.Value = tempreqId
                ...
                ElseIf CType(Session("ExcursionRequestState"), String) = "EditRow" Then
                    ...
                    ''added shahul to check invoice against receipt made or not.
                    Dim strSql1 As String = ""

                    If Trim(txtInvoiceNo.Text) <> "" Then

                        strSql1 = objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), "select 't' from open_detail(nolock) where tran_id='" & Trim(txtInvoiceNo.Text) & "'")
                        If strSql1 <> "" Then
...
#End Region
    ...
    Private Function CreateDataSource(ByVal lngcount As Long) As DataView
        ...
        Try
            ...
            dt.Columns.Add(New DataColumn("cancelcharges", GetType(String)))
            dt.Columns.Add(New DataColumn("noshowcharges", GetType(String)))
            dt.Columns.Add(New DataColumn("canceldeadline", GetType(String)))
            dt.Columns.Add(New DataColumn("waitingpolicy", GetType(String)))
...
#End Region
    ...
    Private Function ValidateMulticost(ByVal lineno As Integer, ByVal epartycode As String, ByVal excid As String, ByRef aiErrorType As Integer) As Boolean
        aiErrorType = 0
        Try
            'For i = 0 To dtExcursionDetails.Rows.Count - 2
            If epartycode = "[Select]" Then
                strSqlQry = "select ISNULL(costvalue,0) costvalue from excursions_detail inner join othgrpmast on excursions_detail.othgrpcode = othgrpmast.othgrpcode where excursions_detail.excid='" & excid & "' and rlineno=" & lineno
                Dim mCostVal As Integer = Val(objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), strSqlQry))
                If mCostVal > 0 Then
                    strSqlQry = "SELECT COUNT(*) from excursions_cost_detail_temp Where excid='" & excid & "' and rlineno=" & lineno
                    Dim mCount As Integer = Val(objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), strSqlQry))
                    If mCount = 0 Then
                        aiErrorType = 1
                        Return False
                    End If
                End If
            Else 'changed by mohamed on 13/04/2016 as to check if not [Select]
                strSqlQry = "SELECT COUNT(*) from excursions_cost_detail_temp Where excid='" & excid & "' and rlineno=" & lineno
                Dim mCount As Integer = Val(objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), strSqlQry))
                If mCount > 0 Then
                    aiErrorType = 2
        ...
        End Try
    End Function
    Private Function ValidateMulticostnew(ByVal lineno As Integer, ByVal epartycode As String, ByVal excid As String, ByVal costvalue As Double, ByRef aiErrorType As Integer) As Boolean
        aiErrorType = 0
        Try
            'For i = 0 To dtExcursionDetails.Rows.Count - 2
            If epartycode = "[Select]" Then
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            If Page.IsValid = True Then
                ...
                Dim liErrorType As Integer
                ''' Added 22/02/16 
                If CType(Session("ExcursionRequestSubEntryState"), String) = "EditRow" Then
                    For i = 0 To dtExcursionDetails.Rows.Count - 2
                        liErrorType = 0
                        If ValidateMulticost(CType(dtExcursionDetails.Rows(i)("rlineno"), Integer), dtExcursionDetails.Rows(i)("epartycode").ToString, txtExcursionID.Text, liErrorType) = False Then
                            If liErrorType = 2 Then
                                ErrMsg = CType(dtExcursionDetails.Rows(i)("othtypcode").ToString, String) + " In this tour, Please remove multicost Values "
                            Else
                                ErrMsg = CType(dtExcursionDetails.Rows(i)("othtypcode").ToString, String) + " In this tour, Please enter multicost Values "
                            End If

                            Dim erralert As String = "alert('" & ErrMsg & "');"
                            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", erralert, True)
                            'ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Please enter multicost values');", True)
                            Exit Sub
                        End If
                    Next
                End If
                If CType(Session("ExcursionRequestSubEntryState"), String) = "AddRow" Then
                    For i = 0 To dtExcursionDetails.Rows.Count - 2
                        liErrorType = 0
                        If ValidateMulticostnew(CType(dtExcursionDetails.Rows(i)("rlineno"), Integer), dtExcursionDetails.Rows(i)("epartycode").ToString, hdnExcursionID.Value, CType(dtExcursionDetails.Rows(i)("costAmountAED"), Double), liErrorType) = False Then
                            If liErrorType = 2 Then
                                ErrMsg = CType(dtExcursionDetails.Rows(i)("othtypcode").ToString, String) + " In this tour, Please remove multicost Values "
                            Else
                                ErrMsg = CType(dtExcursionDetails.Rows(i)("othtypcode").ToString, String) + " In this tour, Please enter multicost Values "
                ...
                If CType(Session("ExcursionRequestSubEntryState"), String) = "AddRow" Then
                    ...
                    If hdnExcursionID.Value <> "" Then
                        tempreqId = hdnExcursionID.Value
                    End If
                    ...
                    'For i = 0 To dtExcursionDetails.Rows.Count - 2
                    '    If ValidateMulticost(RLineNo, dtExcursionDetails.Rows(i)("epartycode").ToString, hdnExcursionID.Value) = False Then
                    '        ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('Please enter multiple cost values');", True)
                    '        Exit Sub
                    '    End If
                    'Next
                    ...
                    For i = 0 To dtExcursionDetails.Rows.Count - 2
                        ...
                        SqlCmd.Parameters.Add(New SqlParameter("@confno", SqlDbType.VarChar, 100)).Value = IIf(dtExcursionDetails.Rows(i)("confno").ToString = "", txtTicketNo.Text.Trim, dtExcursionDetails.Rows(i)("confno").ToString)
                        ...
                        SqlCmd.Parameters.Add(New SqlParameter("@cancelcharges", SqlDbType.VarChar, 250)).Value = dtExcursionDetails.Rows(i)("cancelcharges").ToString
                        SqlCmd.Parameters.Add(New SqlParameter("@noshowcharges", SqlDbType.VarChar, 250)).Value = dtExcursionDetails.Rows(i)("noshowcharges").ToString
                        SqlCmd.Parameters.Add(New SqlParameter("@canceldeadline", SqlDbType.VarChar, 250)).Value = dtExcursionDetails.Rows(i)("canceldeadline").ToString
                        SqlCmd.Parameters.Add(New SqlParameter("@waitingpolicy", SqlDbType.VarChar, 500)).Value = dtExcursionDetails.Rows(i)("waitingpolicy").ToString
                    ...
                    RLineNo = 1
                    '  SaveExcursionCostDetails(hdnExcursionID.Value.Trim)

                    'Dim mStrQry As String = " SELECT * FROM excursions_cost_detail_temp(nolock) Where excid ='" & hdnExcursionID.Value.Trim & "'"
                    Dim mStrQry As String = " SELECT * FROM excursions_cost_detail_temp(nolock) Where excid ='" & tempreqId.Trim & "'"
                    ...
                    If Not Session("ExcursionCostDetailTable") Is Nothing Then
                        ...
                        '   SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
                        ''commented shahul 22/02/16
                        'SqlCmd = New SqlCommand("DELETE FROM Excursions_CostDetail Where excid='" & CType(hdnExcursionID.Value.Trim, String) & "'", mySqlConn, sqlTrans)
                        'SqlCmd.CommandType = CommandType.Text
                        'SqlCmd.ExecuteNonQuery()
                        '''''''''''''''''''
                        ...
                        For Each row As DataRow In dtExcCostDetails.Rows
                            ...
                            SqlCmd.Parameters.Add(New SqlParameter("@excid", SqlDbType.VarChar, 20)).Value = CType(hdnExcursionID.Value.Trim, String)
                            ...
                            '  SqlCmd.Parameters.Add(New SqlParameter("@tempreqid", SqlDbType.VarChar, 20)).Value = CType(tempreqId.Trim, String)
                        ...
                        If flag Then
                            ...
                            'delQry = "DELETE FROM excursions_cost_detail_temp Where excid ='" & hdnExcursionID.Value.Trim & "'"
                            delQry = "DELETE FROM excursions_cost_detail_temp Where excid ='" & tempreqId.Trim & "'"
                            '   SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
                            SqlCmd = New SqlCommand(delQry, mySqlConn, sqlTrans)
                            SqlCmd.CommandType = CommandType.Text
                            SqlCmd.ExecuteNonQuery()
                        End If
                    End If
                    ...
                    ' '''''update invoice no Header table

                    'Dim invoiceno As String = ""

                    'invoiceno = objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), "select isnull(invoiceno,'') from  reservation_invoice_header(nolock)  where  requestid='" & hdnExcursionID.Value.Trim & "' ")
                    'If Not invoiceno Is Nothing Then

                    '    If invoiceno <> "" And objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), "select isnull(invno,'')  from  excursions_header(nolock)  where  excid='" & hdnExcursionID.Value.Trim & "' ") = "" Then
                    '        Dim parms12 As New List(Of SqlParameter)
                    '        Dim parm12(2) As SqlParameter
                    '        Dim parm As New List(Of SqlParameter)

                    '        SqlCmd.CommandType = CommandType.StoredProcedure

                    '        SqlCmd.CommandText = "sp_update_Invoiceno"

                    '        parm(0) = New SqlParameter("@requestid", CType(hdnExcursionID.Value.Trim, String))
                ...
                If CType(Session("ExcursionRequestSubEntryState"), String) = "EditRow" Then
                    ...
                    For i = 0 To dtExcursionDetails.Rows.Count - 2
                        ...
                        SqlCmd.Parameters.Add(New SqlParameter("@provby", SqlDbType.VarChar, 20)).Value = ""
                        SqlCmd.Parameters.Add(New SqlParameter("@attn", SqlDbType.VarChar, 100)).Value = dtExcursionDetails.Rows(i)("attn").ToString
                        SqlCmd.Parameters.Add(New SqlParameter("@confno", SqlDbType.VarChar, 100)).Value = IIf(dtExcursionDetails.Rows(i)("confno").ToString = "", txtTicketNo.Text.Trim, dtExcursionDetails.Rows(i)("confno").ToString) ' dtExcursionDetails.Rows(i)("confno").ToString
                        SqlCmd.Parameters.Add(New SqlParameter("@conflineno", SqlDbType.Int)).Value = 0
                        SqlCmd.Parameters.Add(New SqlParameter("@confirmedon", SqlDbType.DateTime)).Value = DBNull.Value
                    ...
                    Dim mStrQry As String = " SELECT * FROM excursions_cost_detail_temp(nolock) Where excid ='" & txtExcursionID.Text.Trim & "'"
                    Dim mDt As DataSet = objUtils.ExecuteQuerySqlnew(Session("dbconnectionName"), mStrQry)
                    If mDt.Tables.Count > 0 Then Session("ExcursionCostDetailTable") = mDt.Tables(0)
                    If Not Session("ExcursionCostDetailTable") Is Nothing Then
                        Dim flag As Boolean = False
                        ...
                        ''' commeneted shahul 22/02/2016
                        'SqlCmd = New SqlCommand("DELETE FROM Excursions_CostDetail Where excid='" & CType(txtExcursionID.Text.Trim, String) & "'", mySqlConn, sqlTrans)
                        'SqlCmd.CommandType = CommandType.Text
                        'SqlCmd.ExecuteNonQuery()
                        ...
                        For Each row As DataRow In dtExcCostDetails.Rows
                            ...
                            SqlCmd.Parameters.Add(New SqlParameter("@RowId", SqlDbType.Int)).Value = CType(row.Item("rowid"), Integer) '0
                            SqlCmd.Parameters.Add(New SqlParameter("@excid", SqlDbType.VarChar, 20)).Value = CType(txtExcursionID.Text.Trim, String)
                            SqlCmd.Parameters.Add(New SqlParameter("@rlineno", SqlDbType.Int)).Value = CType(row.Item("rlineno"), Integer)
                            SqlCmd.Parameters.Add(New SqlParameter("@othtypcode", SqlDbType.VarChar, 20)).Value = CType(row.Item("othtypcode"), String)
                            SqlCmd.Parameters.Add(New SqlParameter("@supplier", SqlDbType.VarChar, 20)).Value = CType(row.Item("supplier"), String)
                            SqlCmd.Parameters.Add(New SqlParameter("@AdultCostRate", SqlDbType.Decimal)).Value = CType(row.Item("AdultCostRate"), Decimal)
                            ...
                            SqlCmd.Parameters.Add(New SqlParameter("@noofunits", SqlDbType.Int)).Value = CType(row.Item("noofunits"), Integer)
                            SqlCmd.Parameters.Add(New SqlParameter("@remarks", SqlDbType.VarChar, 100)).Value = CType(row.Item("remarks"), String)
                            '  SqlCmd.Parameters.Add(New SqlParameter("@tempreqid", SqlDbType.VarChar, 20)).Value = CType(txtExcursionID.Text.Trim, String)
                        ...
                        If flag Then
                            Dim delQry As String = String.Empty
                            delQry = "DELETE FROM excursions_cost_detail_temp Where excid ='" & txtExcursionID.Text.Trim & "'"
                            '   SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
                            SqlCmd = New SqlCommand(delQry, mySqlConn, sqlTrans)
                            SqlCmd.CommandType = CommandType.Text
                            SqlCmd.ExecuteNonQuery()
                    ...
                    invoiceno = objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), "select isnull(invoiceno,'') from  reservation_invoice_header(nolock)  where  requestid='" & txtExcursionID.Text.Trim & "' ")
                    If Not invoiceno Is Nothing Then

                        If invoiceno <> "" And objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), "select isnull(invno,'')  from  excursions_header(nolock)  where  excid='" & txtExcursionID.Text.Trim & "' ") = "" Then
                            Dim parms12 As New List(Of SqlParameter)
                            Dim parm12(2) As SqlParameter
                            Dim parm As New List(Of SqlParameter)
                            ...
                            parm12(0) = New SqlParameter("@requestid", CType(txtExcursionID.Text.Trim, String))
                            parm12(1) = New SqlParameter("@invoiceno", CType(invoiceno, String))
                            SqlCmd.Parameters.Clear()
                            For i = 0 To 1
                                SqlCmd.Parameters.Add(parm12(i))
                            Next
                            SqlCmd.ExecuteNonQuery()
    ...
    Private Function SaveExcursionCostDetails(ByVal excId As String, Optional ByVal rlineno As Integer = 0) As Boolean
        Try
            Dim mStrQry As String = " SELECT * FROM excursions_cost_detail_temp(nolock) Where excid ='" & excId & "'"
            Dim mDt As DataSet = objUtils.ExecuteQuerySqlnew(Session("dbconnectionName"), mStrQry)
            If mDt.Tables.Count > 0 Then Session("ExcursionCostDetailTable") = mDt.Tables(0)
            If Not Session("ExcursionCostDetailTable") Is Nothing Then
                Dim flag As Boolean = False
    ...
    Protected Sub gvExcursionRequest_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvExcursionRequest.RowCommand
        Try
            ...
            Session("exCurrCodeMhd") = ddlCurrency.Value 'changed by mohamed on 01/05/2016
            ...
            If e.CommandName = "AddRow" Then
                ...
                If txtExcursionID.Text = "" Then
                    ...
                    'Dim mdelQry As String = "DELETE FROM Excursions_Cost_detail_temp Where excid='" & Session("Excursion_ID") & "' and rlineno='" & lblId.Text & "'"
                    'SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
                    'SqlCmd = New SqlCommand(mdelQry, SqlConn)
                    'SqlCmd.CommandType = CommandType.Text
                    'SqlCmd.ExecuteNonQuery()
            ...
            ElseIf e.CommandName = "EditRow" Then
                ...
                Dim strpop As String = ""
                strpop = "window.open('ExcursionRequestSubEntry.aspx?State=EditRow&IsView=" + IsView + "&RefCode=" + CType(Session("ManageTicketRefCode"), String) + "'&LineNo=" + CType(lblId.Text, Integer) + ",'ExcursionRequestSubEntry','width=1000,height=620 left=20,top=20 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
    ...
    Private Sub CreateDataSourceSubEntry()
        Try
            ...
            dtExcursionDetails.Columns.Add(New DataColumn("cancelcharges", GetType(String)))
            dtExcursionDetails.Columns.Add(New DataColumn("noshowcharges", GetType(String)))
            dtExcursionDetails.Columns.Add(New DataColumn("canceldeadline", GetType(String)))
            dtExcursionDetails.Columns.Add(New DataColumn("waitingpolicy", GetType(String)))
    ...
    Private Sub ShowExcursionRequest_Details(ByVal RefCode As String)
        ...
        Try
           ...
           "excursions_detail.cancelcharges,excursions_detail.noshowcharges,excursions_detail.canceldeadline,excursions_detail.waitingpolicy " & _
    ...
    Protected Sub btnExit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExit.Click
        Dim strscript As String = ""
        'Dim mdelQry As String = "DELETE FROM Excursions_Cost_detail_temp Where excid='" & Session("Excursion_ID") & "'"
        'SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
        'SqlCmd = New SqlCommand(mdelQry, SqlConn)
        'SqlCmd.CommandType = CommandType.Text
        'SqlCmd.ExecuteNonQuery()
    ...
    Public Function SendMailtotrfdept(ByVal requestid As String) As Boolean
        ...
        Try
            ...
            strSql = "SELECT trf_required,excid,rlineno,othtypcode,guestname FROM  excursions_detail where excid='" & requestid & "' and trf_required = 1"
            ...
            If ds.Tables.Count > 0 Then
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    For i = 0 To ds.Tables(0).Rows.Count - 1
                        ...
                        strcust = "<table style='font-family: Verdana'><tr><td> There is a request for transfer in Excursion Booking –" + requestid + "  Exc Code.-" + CType(ds.Tables(0).Rows(i)("othtypcode"), String) + "  Guest Name.-" + CType(ds.Tables(0).Rows(i)("guestname"), String) + "</td></tr>"
    ...
    Public Shared Function GetBaseCurrency(ByVal constr As String) As String        
        Dim mObjUtilities As New clsUtils
        Dim result As String
        Dim sqlstr As String
        sqlstr = "SELECT RTrim(option_selected) currency FROM reservation_parameters Where param_id=457"
        result = mObjUtilities.ExecuteQueryReturnStringValuenew(constr, sqlstr)
        Return result        
    End Function
