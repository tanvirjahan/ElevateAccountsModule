        ...
        If Page.IsPostBack = False Then
            Try
                ...
                If hdOPMode.Value = "N" Then
                    lblHeading.Text = "Add Sector Groups"
                ...
                ElseIf hdOPMode.Value = "E" Then
                    ...
                    Page.Title = Page.Title + " " + "Edit Sector Group"
                    btnSave.Text = "Update"
                ...
                Else
                    objUser.CheckNewFormatUserRight(Session("dbconnectionName"), CType(Session("GlobalUserName"), String), CType(Session("Userpwd"), String), _
                                                CType(strappname, String), "ExcursionModule\ExcSectorGroups.aspx?appid=" + strappid, btnNew, btnEdit, _
...
#End Region
    ''' <summary>
    ''' GetHotelGroup #Region "Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click"
        ...
        Try
            ...
            If Page.IsValid = True Then
                ...
                If hdOPMode.Value = "N" Or hdOPMode.Value = "E" Then
                    ...
                    If hdOPMode.Value = "N" Then
                        mySqlCmd = New SqlCommand("sp_add_sectorgrps", mySqlConn, sqlTrans)

                        lastno = objUtils.ExecuteQueryReturnSingleValuenew(CType(Session("dbconnectionName"), String), "select lastno from docgen where optionname='EXCSECGRP'")
                        optionval = objUtils.GetAutoDocNo("EXCSECGRP", mySqlConn, sqlTrans)
                        txtGroupCode.Text = optionval.Trim

                    ElseIf hdOPMode.Value = "E" Then
                        mySqlCmd = New SqlCommand("sp_mod_sectorgrps", mySqlConn, sqlTrans)
                    ...
                    mySqlCmd.Parameters.Add(New SqlParameter("@ctrycode", SqlDbType.VarChar, 20)).Value = objUtils.GetDBFieldFromStringnew(Session("dbconnectionName"), "citymast", "ctrycode", "citycode", CType(TxtCityCode.Text.Trim, String))
                ...
                ElseIf hdOPMode.Value = "D" Then
                    ...
                    mySqlCmd = New SqlCommand("sp_del_sectorgrps", mySqlConn, sqlTrans)
                    mySqlCmd.CommandType = CommandType.StoredProcedure
                    mySqlCmd.Parameters.Add(New SqlParameter("@sectorgroupcode", SqlDbType.VarChar, 20)).Value = CType(txtGroupCode.Text.Trim, String)
                    mySqlCmd.Parameters.Add(New SqlParameter("@ValidXMLInput", SqlDbType.Xml)).Value = strBuffer.ToString
                    mySqlCmd.Parameters.Add(New SqlParameter("@userlogged", SqlDbType.VarChar, 10)).Value = CType(Session("GlobalUserName"), String)
...
#Region "Public Function checkForDuplicate() As Boolean"
    Public Function checkForDuplicate() As Boolean
        ...
        If hdOPMode.Value = "N" Then
            ...
            If objUtils.isDuplicatenew(Session("dbconnectionName"), "othtypmast", "othtypname", ((Trim(txtGroupTagName.Text.Trim).Replace("(C)", "")).Replace("(SG)", "")).Replace("(G)", "")) Then
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('This group name is already present.');", True)
                checkForDuplicate = False
                Exit Function
            End If
        ElseIf hdOPMode.Value = "E" Then
            If objUtils.isDuplicateForModifynew(Session("dbconnectionName"), "othtypmast", "othtypcode", "othtypname", CType(((Trim(txtGroupTagName.Text.Trim).Replace("(C)", "")).Replace("(SG)", "")).Replace("(HG)", ""), String), txtGroupCode.Text.Trim) Then
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('This group name is already present.');", True)
                checkForDuplicate = False
                Exit Function
            End If
...
#End Region
    ...
    Protected Sub btnNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNew.Click
        ...
        txtGroupCode.Enabled = False
        txtGroupTagName.Focus()
        lblHeading.Text = "Add Sector Group"
        btnSave.Text = "Save"
        Dim col As System.Drawing.Color
        col = System.Drawing.ColorTranslator.FromHtml("#2D7C8A")
    ...
    Public Shared Function GetSectorGroups(ByVal prefixText As String) As List(Of String)
        ...
        Try
            ...
            strSqlQry = "select rtrim(ltrim(othtypname))+'(SG)' name from othtypmast where othgrpcode= '" & CType(HttpContext.Current.Session("trfgroupcode"), String) & "' and  othtypname like  " & "'" & prefixText & "%'  order by name"
            'and grouptype<>'C'
            Dim SqlConn As New SqlConnection
            Dim myDataAdapter As New SqlDataAdapter
            ' SqlConn = clsDBConnect.dbConnectionnew(objclsConnectionName.ConnectionName)
            ...
            If myDS.Tables(0).Rows.Count > 0 Then
                For i As Integer = 0 To myDS.Tables(0).Rows.Count - 1
                    sectorgrpnames.Add(myDS.Tables(0).Rows(i)("name").ToString())
                Next
