<%@ Page Language="VB" MasterPageFile="~/SubPageMaster.master" AutoEventWireup="false" CodeFile="ExcursionSellingTypes.aspx.vb" Inherits="Other_Services_Selling_Types"  %>
...
if ((document.getElementById("<%=TxtOtherServiceCode.ClientID%>").value=="")||(document.getElementById("<%=txtOtherServiceSelling.ClientID%>").value=="")||(document.getElementById("<%=ddlCurrencyCd.ClientID%>").value=="[Select]"))
{
 ...
 {
       if (state=='New'){if(confirm('Are you sure you want to Save Excursion Selling Types?')==false)return false;}
       if (state=='Edit'){if(confirm('Are you sure you want to Update Excursion Selling Types?')==false)return false;}
       if (state == 'Delete') { if (confirm('Are you sure you want to Delete Excursion  Selling Types?') == false) return false; }
 }
}  
...
function  GetValueCode()
{
    ...
    <asp:UpdatePanel id="UpdatePanel1" runat="server">
        <contenttemplate>
<TABLE style="BORDER-RIGHT: gray 2px solid; BORDER-TOP: gray 2px solid; BORDER-LEFT: gray 2px solid; WIDTH: 656px; BORDER-BOTTOM: gray 2px solid"><TBODY><TR><TD class="td_cell" align=center colSpan=2><asp:Label id="lblHeading" runat="server" Text="Add New Excursion  Selling Types" CssClass="field_heading" Width="645px"></asp:Label></TD></TR><TR style="COLOR: #ff0000"><TD style="WIDTH: 209px" class="td_cell"><SPAN style="COLOR: #000000">Code </SPAN><SPAN style="COLOR: #ff0000" class="td_cell">*</SPAN></TD><TD style="COLOR: #000000"><INPUT style="WIDTH: 196px" id="TxtOtherServiceCode" class="txtbox" tabIndex=1 type=text maxLength=150 runat="server" /></TD></TR><TR><TD style="WIDTH: 209px; HEIGHT: 24px" class="td_cell">
