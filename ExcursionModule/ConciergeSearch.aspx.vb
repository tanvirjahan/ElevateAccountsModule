            ...
            Try
                ...
                Else
                    objUser.CheckUserRight(Session("dbconnectionName"), CType(Session("GlobalUserName"), String), CType(Session("Userpwd"), String), _
                                             CType(strappname, String), "ExcursionModule\ConciergeSearch.aspx?appid=" + strappid, btnAddNew, btnExportToExcel, _
    ...
    Private Function BuildCondition() As String
        ...
        If txtcitycode.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                ...
                strWhereCond = " upper(spersonmast.spersoncode) LIKE '" & Trim(txtcitycode.Text.Trim.ToUpper) & "%'"
            Else
                strWhereCond = strWhereCond & " AND upper(spersonmast.spersoncode) LIKE '" & Trim(txtcitycode.Text.Trim.ToUpper) & "%'"
            End If
        End If
        ...
        If txtcityname.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                ...
                strWhereCond = " upper(spersonmast.spersonname) LIKE '" & Trim(txtcityname.Text.Trim.ToUpper) & "%'"
            Else
                strWhereCond = strWhereCond & " AND upper(spersonmast.spersonname) LIKE '" & Trim(txtcityname.Text.Trim.ToUpper) & "%'"
            End If
        End If
        ...
        If ddldeptcode.Value <> "[Select]" Then
            If Trim(strWhereCond) = "" Then
                strWhereCond = " upper(spersonmast.deptcode) = '" & Trim(ddldeptcode.Items(ddldeptcode.SelectedIndex).Text.Trim.ToUpper) & "'"
            Else
                strWhereCond = strWhereCond & " AND upper(spersonmast.deptcode) = '" & Trim(ddldeptcode.Items(ddldeptcode.SelectedIndex).Text.Trim.ToUpper) & "'"
            End If
        End If
...
#Region "Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = 'ASC')"
    Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = "ASC")
        ...
        Try
            ...
            strSqlQry = "SELECT spersoncode,spersonname,deptcode,adddate,case when isnull(spersonmast.active,0)=1 then 'Active' when isnull(spersonmast.active,0)=0 then 'InActive' end as Active FROM spersonmast "


            If Trim(BuildCondition) <> "" Then
                strSqlQry = strSqlQry & " WHERE " & BuildCondition() & " ORDER BY " & strorderby & " " & strsortorder
            Else
                strSqlQry = strSqlQry & " ORDER BY spersonmast." & strorderby & " " & strsortorder
            End If
...
#Region "Protected Sub btnAddNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddNew.Click"
    Protected Sub btnAddNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddNew.Click
        ...
        strpop = "window.open('concierge.aspx?State=New','Cities','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
    ...
    Protected Sub gv_SearchResult_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gv_SearchResult.RowCommand
        Try
            ...
            If e.CommandName = "EditRow" Then
                ...
                Dim strpop As String = ""
                strpop = "window.open('concierge.aspx?State=Edit&RefCode=" + CType(lblId.Text.Trim, String) + "','Cities','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            ElseIf e.CommandName = "View" Then
                ...
                'Response.Redirect("Cities.aspx", False)
                Dim strpop As String = ""
                strpop = "window.open('concierge.aspx?State=View&RefCode=" + CType(lblId.Text.Trim, String) + "','Cities','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            ElseIf e.CommandName = "DeleteRow" Then
                ...
                'Response.Redirect("Cities.aspx", False)
                Dim strpop As String = ""
                strpop = "window.open('concierge.aspx?State=Delete&RefCode=" + CType(lblId.Text.Trim, String) + "','Cities','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            End If
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objUtils.WritErrorLog("conciergesearch.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        End Try
    End Sub
#End Region
    ...
    Protected Sub btnExportToExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExportToExcel.Click
        ...
        Try
            If gv_SearchResult.Rows.Count <> 0 Then
                ...
                strSqlQry = "SELECT spersoncode [Sales Person Code],spersonname [Sales Person Name],deptcode [Department Code],adddate [Date Created],case when isnull(spersonmast.active,0)=1 then 'Active' when isnull(spersonmast.active,0)=0 then 'InActive' end as Active FROM spersonmast "
                If Trim(BuildCondition) <> "" Then
...
#Region "Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click"
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            Dim strpop As String = ""
            strpop = "window.open('rptReportNew.aspx?Pageame=concierge&BackPageName=conciergeSearch.aspx&code=" & txtcitycode.Text.Trim & "&name=" & txtcityname.Text.Trim & "&deptcode=" & Trim(ddldeptcode.Items(ddldeptcode.SelectedIndex).Text) & "&deptname=" & Trim(ddldeptname.Items(ddldeptname.SelectedIndex).Text) & "','RepCountry','width=1000,height=620 left=20,top=20 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
