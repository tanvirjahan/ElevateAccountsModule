...
Partial Class rptProfitability
...
#Region "Protected Sub btnLoadReport_Click(sender As Object, e As System.EventArgs) Handles btnLoadReport.Click"
    Protected Sub btnLoadReport_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnLoadReport.Click
        Try
            ...
            Else
                SummaryReport(InvDt, NonInvDt, gopDt)
            End If
    ...
    Protected Sub DetailsReport(ByVal resultDt As DataTable, ByVal TotalDt As DataTable, ByVal InvDt As DataTable, ByVal NonInvDt As DataTable, ByVal gopDt As DataTable)
        Try
            If resultDt.Rows.Count > 0 Then
                ...
                Dim wsprofit As IXLWorksheet = wbook.Worksheets.Add("Profitability Report")
                ...
                rngTitle = wsprofit.Range("A" + Convert.ToString(excelLastRow) + ":H" + Convert.ToString(excelLastRow))
                rngTitle.Merge()
                ...
                rngap = wsprofit.Range("A" + Convert.ToString(excelLastRow) + ":H" + Convert.ToString(excelLastRow))
                rngap.Merge()
                ...
                rngSubTitle = wsprofit.Range("A" + Convert.ToString(excelLastRow) + ":H" + Convert.ToString(excelLastRow))
                rngSubTitle.Merge()
                wsprofit.Cell(excelLastRow, 1).Value = "Profitability Report"
                wsprofit.Cell(excelLastRow, 1).Style.Alignment.SetVertical(XLAlignmentVerticalValues.Center).Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center)
                ...
                rngDtTitle = wsprofit.Range("A" + Convert.ToString(excelLastRow) + ":H" + Convert.ToString(excelLastRow + 1))
                rngDtTitle.Merge()
...
#Region "Protected Sub MakeDetailExcel(ByRef wsprofit As IXLWorksheet, ByRef excelLastRow As Integer, ByVal resultDt As DataTable, ByVal TotalDt As DataTable, ByVal InvDt As DataTable, ByVal NonInvDt As DataTable, ByVal gopDt As DataTable, Optional ByVal accountType As String = "")"
    Protected Sub MakeDetailExcel(ByRef wsprofit As IXLWorksheet, ByRef excelLastRow As Integer, ByVal resultDt As DataTable, ByVal TotalDt As DataTable, ByVal InvDt As DataTable, ByVal NonInvDt As DataTable, ByVal gopDt As DataTable, Optional ByVal accountType As String = "")
        ...
        Dim TblTitle() As String = {"Invoice No.", "Invoice Date", "File No.", "Agent Name", "Sales Value" + vbCr + "Excl. VAT", "Cost Value" + vbCr + "Excl. VAT", "Profit", "%"}
        ...
        If groupDr.Count > 0 Then
            For i = 0 To groupDr.Count - 1
                ...
                Dim filterName As String = (From n In resultDt.AsEnumerable() Where n.Field(Of String)("groupby").ToUpper() = filterId Select n.Field(Of String)("groupname")).Distinct.FirstOrDefault()
                ...
                Dim filterDt As DataTable = (From n In resultDt.AsEnumerable() Where n.Field(Of String)("groupby").ToUpper() = filterId Select n).CopyToDataTable()
...
#Region "Protected Sub SummaryReport(ByVal InvDt As DataTable, ByVal NonInvDt As DataTable)"
    Protected Sub SummaryReport(ByVal InvDt As DataTable, ByVal NonInvDt As DataTable, ByVal gopDt As DataTable)
        Try
            Dim excelLastRow As Integer
            Dim wbook As XLWorkbook = New XLWorkbook
            Dim companyname As String = CType(Session("CompanyName"), String)
            ...
            excelLastRow = 1
            Dim rngTitle As IXLRange
            rngTitle = wsprofit.Range("A" + Convert.ToString(excelLastRow) + ":E" + Convert.ToString(excelLastRow))
            rngTitle.Merge()
            wsprofit.Cell(excelLastRow, 1).Value = companyname
            wsprofit.Cell(excelLastRow, 1).Style.Alignment.SetVertical(XLAlignmentVerticalValues.Center).Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center)
            wsprofit.Cell(excelLastRow, 1).Style.Font.SetBold(True).Font.SetFontSize(14)
            ...
            excelLastRow = 2
            Dim rngap As IXLRange
            rngap = wsprofit.Range("A" + Convert.ToString(excelLastRow) + ":E" + Convert.ToString(excelLastRow))
            rngap.Merge()
            wsprofit.Row(excelLastRow).Height = 5
            ...
            excelLastRow = 3
            Dim rngSubTitle As IXLRange
            rngSubTitle = wsprofit.Range("A" + Convert.ToString(excelLastRow) + ":E" + Convert.ToString(excelLastRow))
            rngSubTitle.Merge()
            wsprofit.Cell(excelLastRow, 1).Value = "Profitability Report"
            wsprofit.Cell(excelLastRow, 1).Style.Alignment.SetVertical(XLAlignmentVerticalValues.Center).Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center)
            wsprofit.Cell(excelLastRow, 1).Style.Font.SetBold(True).Font.SetFontSize(12)
            ...
            excelLastRow = 5
            Dim rngDtTitle As IXLRange
            rngDtTitle = wsprofit.Range("A" + Convert.ToString(excelLastRow) + ":E" + Convert.ToString(excelLastRow + 1))
            rngDtTitle.Merge()
            wsprofit.Cell(excelLastRow, 1).Value = "From Date :" + Convert.ToDateTime(txtFromDt.Text).ToString("dd/MM/yyyy") + " - To Date :" + Convert.ToDateTime(txtToDt.Text).ToString("dd/MM/yyyy") + " And Report Group by : " + ddlGroupBy.SelectedItem.Text + " wise"
            wsprofit.Cell(excelLastRow, 1).Style.Alignment.SetVertical(XLAlignmentVerticalValues.Top).Alignment.SetHorizontal(XLAlignmentHorizontalValues.Left)
            wsprofit.Cell(excelLastRow, 1).Style.Font.SetBold(True).Font.SetFontSize(10)
            ...
            excelLastRow = 8
            If ddlGroupBy.SelectedValue <> "Account" Then
                MakeSummaryExcel(wsprofit, excelLastRow, InvDt, NonInvDt, gopDt)
            Else
                Dim acctCode As String = ""
                Dim acctOrder As Integer = 0
                If txtAcctCode.Text.Trim <> "" Then
                ...
                If filterIncome.Count > 0 And acctOrder <> 4 Then
                    ...
                    wsprofit.Cell(excelLastRow, 1).Style.Font.SetFontColor(XLColor.Black)
                    excelLastRow += 1
                    MakeSummaryExcel(wsprofit, excelLastRow, filterInvDt, filterNonInvDt, filterGopDt, "Income")
                End If
                ...
                If filterCostSale.Count > 0 And acctOrder <> 3 Then
                    ...
                    wsprofit.Cell(excelLastRow, 1).Style.Font.SetFontColor(XLColor.Black)
                    excelLastRow += 1
                    MakeSummaryExcel(wsprofit, excelLastRow, filterInvDt, filterNonInvDt, filterGopDt, "CostOfSale")
                End If
            End If

            wsprofit.Columns("1:8").AdjustToContents()
            wsprofit.Columns("2:5").Width = 17
            wsprofit.Columns("2:3").Style.Alignment.SetWrapText(True)

            Dim FileNameNew As String = "ProfitSummaryPrint_" & Now.Year & Now.Month.ToString("00") & Now.Day.ToString("00") & Now.Hour & Now.Minute & Now.Second & ".xlsx"
            Using MyMemoryStream As New MemoryStream()
                wbook.SaveAs(MyMemoryStream)
                wbook.Dispose()
                Response.Clear()
...
#Region "Protected Sub MakeSummaryExcel(ByRef wsprofit As IXLWorksheet, ByRef excelLastRow As Integer, ByVal InvDt As DataTable, ByVal NonInvDt As DataTable, ByVal gopDt As DataTable, Optional ByVal accountType As String = "")"
    Protected Sub MakeSummaryExcel(ByRef wsprofit As IXLWorksheet, ByRef excelLastRow As Integer, ByVal InvDt As DataTable, ByVal NonInvDt As DataTable, ByVal gopDt As DataTable, Optional ByVal accountType As String = "")
        Dim decno As Integer = CType(objUtils.ExecuteQueryReturnSingleValuenew(Session("dbconnectionName"), "select option_selected from reservation_parameters where param_id=509"), Integer)
        Dim decPlaces As String
        If decno = 3 Then
            decPlaces = "#,##0.000;[Red](#,##0.000)"
