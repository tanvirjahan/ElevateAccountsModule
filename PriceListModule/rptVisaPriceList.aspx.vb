...
Partial Class rptVisaPricelist
    ...
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        If Page.IsPostBack = False Then
            Try
                lstOthCatLeft.DataSource = objutils.ExecuteQuerySqlnew(Session("dbconnectionName"), "select othcatcode,othcatname from othcatmast where active <> 0  and othgrpcode  in (select option_selected from reservation_parameters where param_id in (1002))  order by  othcatname ") 'and othgrpcode='TRSPT'
                ...
                'Dim strGrpQry As String = "select othgrpcode,othgrpname from othgrpmast where  active=1 and othmaingrpcode not in (select " & _
                '  " option_selected  from reservation_parameters where param_id in (1001,1104,1105)) order by othgrpcode"
                ...
                strqry = "select othgrpcode,othgrpname from othgrpmast where active=1 And othgrpcode=(Select Option_Selected From Reservation_ParaMeters Where Param_Id='" + Session("OthPolicyFilter") + "') order by othgrpcode"
                ' Dim strGrpQry As String = "select othgrpcode,othgrpname from othgrpmast where  active=1 and othmaingrpcode not in (select " & _
                '   " option_selected  from reservation_parameters where param_id in (1001,1104,1105)) order by othgrpcode"
                objutils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlOtherGroupCode, "othgrpcode", "othgrpname", strqry, True)
                objutils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlOtherGroupName, "othgrpname", "othgrpcode", strqry, True)
                ddlOtherGroupCode.SelectedIndex = 0
                ddlOtherGroupName.SelectedIndex = 0
                ddlOtherGroupCode.Disabled = True
                ddlOtherGroupName.Disabled = True
                ...
                objutils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlSellingCode, "visasellcode", "visasellname", "select visasellcode, visasellname from visasellmast where active=1 order by visasellcode", True)
                objutils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlSellingName, "visasellname", "visasellcode", "select visasellname, visasellcode from visasellmast where active=1 order by visasellname", True)
    ...
    Protected Sub BtnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnPrint.Click
        Try
            If ValidatePage() = True Then
                ...
                If strreportoption <> "VISA" And strreportoption <> "AIRPORTMA" And strreportoption <> "HFEES" Then
                    strreportoption = "OTH"
                End If
                rtptype = "Sales"
                ...
                ' strpop = "window.open('rptVisaPriceListReport.aspx?pageame=rptOtherServicePricelist&BackPageName=rptOtherServicePricelist.aspx&othergroupcode=" & strgrpcode & "&fromdate=" & strfromdate _
        ...
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objutils.WritErrorLog("rptVisaPriceList.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
