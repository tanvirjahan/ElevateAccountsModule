...
#End Region
Partial Class PriceListModule_RoutesSearch
            ...
            Try
                ...
                Else
                    objUser.CheckUserRight(Session("dbconnectionName"), CType(Session("GlobalUserName"), String), CType(Session("Userpwd"), String), _
                                                       CType(strappname, String), "PriceListModule\RoutesSearch.aspx?appid=1", btnAddNew, btnExportToExcel, _
...
#Region "CheckForTransportInOtherTable"
    Sub CheckForTransportInOtherTable()
        Try
            SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
            If SqlConn.State = ConnectionState.Closed Then SqlConn.Open()
            Dim SqlCmd As SqlCommand

            SqlCmd = New SqlCommand("InSert Into othtypmastTransfers   Select Othtypcode,  Case When Transfertype = 0 Then 1 When Transfertype = 1 Then 2 When Transfertype = 2 Then 2 When Transfertype =3 Then 1 End As PickType,Case When Transfertype = 0 Then 2 When Transfertype =1 Then 1 When Transfertype = 2 Then 2 When Transfertype = 3 Then 1 End As DropType,PickUpPoint,'P' From othtypmast Where othgrpcode=(Select Option_Selected From Reservation_ParaMeters Where Param_Id='1001') And pickuppoint <> ''", SqlConn)
            SqlCmd.ExecuteNonQuery()

            SqlCmd = New SqlCommand("InSert Into othtypmastTransfers   Select Othtypcode,  Case When Transfertype = 0 Then 1 When Transfertype = 1 Then 2 When Transfertype = 2 Then 2 When Transfertype =3 Then 1 End As PickType,Case When Transfertype = 0 Then 2 When Transfertype =1 Then 1 When Transfertype = 2 Then 2 When Transfertype = 3 Then 1 End As DropType,dropoffpoint,'D' From othtypmast Where othgrpcode=(Select Option_Selected From Reservation_ParaMeters Where Param_Id='1001') And dropoffpoint <> ''", SqlConn)
            SqlCmd.ExecuteNonQuery()

            SqlCmd = New SqlCommand("Update othtypmast Set transfertype= transfertype + 1 , pickuppoint='',dropoffpoint='' Where pickuppoint <> '' And dropoffpoint <> ''", SqlConn)
            SqlCmd.ExecuteNonQuery()
...
#Region "Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = 'ASC')"
    Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = "ASC")
        ...
        Try
            ...
            '"case dbo.othtypmast.autocancelreq when 1 then 'Yes' else 'No' end as autocancelreq, dbo.othtypmast.adddate, dbo.othtypmast.adduser, dbo.othtypmast.moddate, dbo.othtypmast.moduser " & _
            '"FROM         dbo.othgrpmast INNER JOIN dbo.othtypmast ON dbo.othgrpmast.othgrpcode = dbo.othtypmast.othgrpcode"
            '     ' strSqlQry = "SELECT  Distinct othtypcode, othtypname, Case When  TransferType = 1 Then 'Arrival Borders' When TransferType=2 Then 'Departure Borders' When TransferType = 3 Then 'Internal Transfer/Excursion' When TransferType=4 Then 'Arrival/Departure Transfer Borders' End As TransferType,Case When PickType=1 Then 'Airport / Borders' When PickType=2 Then 'Hotel Sectors' When PickType=3 Then 'Hotels'  End As [Pick Type], Case When DropType=1 Then 'Airport / Borders' When DropType=2 Then 'Hotel Sectors' When DropType=3 Then 'Hotels'  End As [Drop Type],rankorder, minpax, case active when 1 then 'Active'  else 'InActive' end Active, case printconf when 1 then 'Yes' else 'No' end as printconf, case paxcheckreq when 1 then 'Yes' else 'No' end as paxcheckreq, case printremarks when 1 then 'Yes' else 'No' end as printremarks, case autocancelreq when 1 then 'Yes' else 'No' end as autocancelreq, adddate, adduser, moddate, moduser FROM OthtypMastTransfers "


            strSqlQry = "SELECT  dbo.othtypmast.othtypcode, dbo.othtypmast.othtypname, dbo.othtypmast.othgrpcode, dbo.othgrpmast.othgrpname, dbo.othtypmast.rankorder, " & _
                            "        dbo.othtypmast.minpax, case dbo.othtypmast.active when 1 then 'Active'  else 'InActive' end Active, case dbo.othtypmast.printconf when 1 then 'Yes' else 'No' end as printconf, case dbo.othtypmast.paxcheckreq when 1 then 'Yes' else 'No' end as paxcheckreq, case dbo.othtypmast.printremarks when 1 then 'Yes' else 'No' end as printremarks, " & _
                           "case dbo.othtypmast.autocancelreq when 1 then 'Yes' else 'No' end as autocancelreq, dbo.othtypmast.adddate, dbo.othtypmast.adduser, dbo.othtypmast.moddate, dbo.othtypmast.moduser, " & _
                           "case  when othtypmast.shuttle=1 then 'Yes' else 'No' end shuttle " & _
                           "FROM         dbo.othgrpmast INNER JOIN dbo.othtypmast ON dbo.othgrpmast.othgrpcode = dbo.othtypmast.othgrpcode"
...
#Region "Protected Sub btnAddNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddNew.Click"
    Protected Sub btnAddNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddNew.Click
        ...
        strpop = "window.open('Routes.aspx?State=New','Othtype','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
    ...
    Protected Sub gv_SearchResult_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gv_SearchResult.RowCommand
        Try
            ...
            If e.CommandName = "EditRow" Then
                ...
                'Response.Redirect("Routes.aspx", False)
                Dim strpop As String = ""
                strpop = "window.open('Routes.aspx?State=Edit&RefCode=" + CType(lblId.Text.Trim, String) + "','Routes','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            ElseIf e.CommandName = "View" Then
                ...
                'Response.Redirect("Routes.aspx", False)
                Dim strpop As String = ""
                strpop = "window.open('Routes.aspx?State=View&RefCode=" + CType(lblId.Text.Trim, String) + "','Routes','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            ElseIf e.CommandName = "DeleteRow" Then
                ...
                'Response.Redirect("Routes.aspx", False)
                Dim strpop As String = ""
                strpop = "window.open('Routes.aspx?State=Delete&RefCode=" + CType(lblId.Text.Trim, String) + "','Routes','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            End If
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
    ...
    Protected Sub btnExportToExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExportToExcel.Click
        ...
        Try
            If gv_SearchResult.Rows.Count <> 0 Then
                ...
                strSqlQry = "SELECT  dbo.othtypmast.othtypcode, dbo.othtypmast.othtypname, dbo.othtypmast.othgrpcode, dbo.othgrpmast.othgrpname, dbo.othtypmast.rankorder, " & _
      "        dbo.othtypmast.minpax,case  when othtypmast.shuttle=1 then 'Yes' else 'No' end as shuttle , case dbo.othtypmast.active when 1 then 'Active'  else 'InActive' end Active, case dbo.othtypmast.printconf when 1 then 'Yes' else 'No' end as printconf, case dbo.othtypmast.paxcheckreq when 1 then 'Yes' else 'No' end as paxcheckreq, case dbo.othtypmast.printremarks when 1 then 'Yes' else 'No' end as printremarks, " & _
     "case dbo.othtypmast.autocancelreq when 1 then 'Yes' else 'No' end as autocancelreq, dbo.othtypmast.adddate, dbo.othtypmast.adduser, dbo.othtypmast.moddate, dbo.othtypmast.moduser " & _
     "FROM         dbo.othgrpmast INNER JOIN dbo.othtypmast ON dbo.othgrpmast.othgrpcode = dbo.othtypmast.othgrpcode"
                ' strSqlQry = "SELECT  Distinct othtypcode, othtypname, Case When  TransferType = 1 Then 'Arrival Borders' When TransferType=2 Then 'Departure Borders' When TransferType = 3 Then 'Internal Transfer/Excursion' When TransferType=4 Then 'Arrival/Departure Transfer Borders' End As TransferType,Case When PickType=1 Then 'Airport / Borders' When PickType=2 Then 'Hotel Sectors' When PickType=3 Then 'Hotels'  End As [Pick Type], Case When DropType=1 Then 'Airport / Borders' When DropType=2 Then 'Hotel Sectors' When DropType=3 Then 'Hotels'  End As [Drop Type],rankorder, minpax, case active when 1 then 'Active'  else 'InActive' end Active, case printconf when 1 then 'Yes' else 'No' end as printconf, case paxcheckreq when 1 then 'Yes' else 'No' end as paxcheckreq, case printremarks when 1 then 'Yes' else 'No' end as printremarks, case autocancelreq when 1 then 'Yes' else 'No' end as autocancelreq, adddate, adduser, moddate, moduser FROM OthtypMastTransfers "
...
#Region "Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click"
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        ...
        Try
            ...
            'Session.Add("BackPageName", "RoutesSearch.aspx")
            Dim strpop As String = ""
            strpop = "window.open('rptReportNew.aspx?Pageame=Routes&BackPageName=RoutesSearch.aspx&OthtypeCode=" & txtCode.Value.Trim & "&OthtypeName=" & txtName.Value.Trim & "','RepOthType','width=1000,height=620 left=20,top=20 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
...
#End Region
    ...
    Private Sub fillorderby()
        ...
        'ddlOrderBy.Items.Add("Pick Type")
        'ddlOrderBy.Items.Add("Drop Type")
        ddlOrderBy.SelectedIndex = 0
    End Sub
    ...
    Protected Sub btnHelp_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnHelp.Click
        ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "window.open('../Help.aspx?hi=RoutesSearch','_blank','status=1,scrollbars=1,top=135,left=760,width=250,height=500');", True)
    End Sub
End Class
