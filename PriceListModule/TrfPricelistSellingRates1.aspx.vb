...
Partial Class PriceListModule_TrfPricelistSellingRates1
...
#Region " Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init"
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        ViewState.Add("TrfPLsellingRate1codeState1", Request.QueryString("State"))
        ViewState.Add("TrfPLsellingRate1codeCode1", Request.QueryString("RefCode"))
        ...
        If IsPostBack = False Then
            ...
            ShowSellingRadioBtns(ViewState("TrfPLsellingRate1codeCode1"))
            createdatatable()
            dt = Session("TrfGridData")
            Dim fld2 As String = ""
...
#Region "Private Sub createdatatable()"
    ...
    Private Sub createdatatable()
        ...
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objUtils.WritErrorLog("TrfPriceslistSellingRates.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        Finally
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Page.IsPostBack = False Then
            Try
                ...
                ShowDates(ViewState("TrfPLsellingRate1codeCode1"))
                ...
                Try
                    ...
                    If ViewState("TrfPLsellingRate1codeState1") = "New" Then
                        lblHeading.Text = "Transfer Price List - Selling Rates"
                        ...
                        '     gv_SearchResult.Enabled = False 'only viewing , so no need to edit or so
                        ...
                        'End If
                    ElseIf ViewState("TrfPLsellingRate1codeState1") = "Edit" Then
                        lblHeading.Text = "Transfer Price List - Selling Rates"
                        Page.Title = Page.Title + " " + "Transfer Price List - Selling Rates"
                        ...
                        '    gv_SearchResult.Enabled = False
                        'End If
                    ElseIf ViewState("TrfPLsellingRate1codeState1") = "View" Then
                        lblHeading.Text = "View Transfer Price List - Selling Rates"
                        Page.Title = Page.Title + " " + "View Transfer Price List - Selling Rates"
                        ShowDynamicGrid()
                        btnSave.Visible = False

                        gv_SearchResult.Enabled = False
                    ElseIf ViewState("TrfPLsellingRate1codeState1") = "Copy" Then
                        lblHeading.Text = "Transfer Price List - Selling Rates"
                        Page.Title = Page.Title + " " + "Transfer Price List - Selling Rates"
                        ''btnGenerate.Visible = False
                        ''If ChkBManual.Checked = True Then
                ...
                While mySqlReader.Read
                    For Each gvrow In gv_SearchResult.Rows
                        ...
                        If gvrow.Cells(0).Text = Linno Then
                            ...
                            'Else
                            StrQryTemp = "select * from trfplist_selld where tplistcode='" & txtPLCode.Text & "' and selltype='" & lblSellingCode.Text.Trim & "'and oclineno='" & Linno & "'"
                            'End If
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            Try
                        ...
                        For Each gvrow In gv_SearchResult.Rows
                            If n = 0 Then
                                For j = 0 To cnt - 4
                                    If heading(j) = "From Date" Or heading(j) = "To Date" Or heading(j) = "Pkg" Then
                                    Else
                                        mySqlCmd = New SqlCommand("sp_update_trfplistd", mySqlConn, sqlTrans)
                                        ...
                                        If Request.QueryString("sellcode") <> Nothing Then
                                            mySqlCmd.Parameters.Add(New SqlParameter("@selltype", SqlDbType.VarChar, 20)).Value = CType(lblSellingCode.Text, String)
                                        End If

                                         If chkAllow.Checked = True Then
                                            mySqlCmd.Parameters.Add(New SqlParameter("@allowflg", SqlDbType.VarChar, 20)).Value = 1
                                        Else
                                            mySqlCmd.Parameters.Add(New SqlParameter("@allowflg", SqlDbType.VarChar, 20)).Value = 0
                                        End If
                ...
                'Response.Redirect("HederRsellingcode1.aspx", False
                Dim marketstr As String = ""
                ...
                Response.Redirect("TrfPricelistSellingRates.aspx?State=" & CType(ViewState("TrfPLsellingRate1codeState1"), String) & "&RefCode=" & CType(ViewState("TrfPLsellingRate1codeCode1"), String) &
                                  ...
                                  "&CurrencyName=" & ddlCurrencyCode.Value & "&SubSeasonCode=" & ddlSubSeasonCode.Items(ddlSubSeasonCode.SelectedIndex).Text &
                                  "&SubSeasonName=" & ddlSubSeasonName.Items(ddlSubSeasonName.SelectedIndex).Text &
                                  "&PListCode=" & txtPLCode.Text & "&sellcode=" & rdSellingList.SelectedValue &
                                  "&transfertype=" & ddlServerType.SelectedIndex, False)
...
go1:
    ...
    Public Sub fillDategrd(ByVal grd As GridView, Optional ByVal blnload As Boolean = False, Optional ByVal count As Integer = 1)
        Dim lngcnt As Long
        If ViewState("TrfPLsellingRate1codeCode1") <> Nothing Then
            mySqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
            strSqlQry = "select count(*) from trfplisth_dates where tplistcode='" + ViewState("TrfPLsellingRate1codeCode1") + "'"
            mySqlCmd = New SqlCommand(strSqlQry, mySqlConn)
