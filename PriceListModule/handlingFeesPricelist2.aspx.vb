...
#End Region
Partial Class PriceListModule_handlingFeesPricelist2
            ...
            If IsPostBack = False Then
               ...
               objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlGroupCode, "othgrpcode", "othgrpname", strqry, True)
                ...
                ElseIf ViewState("OthpricelistState2") = "View" Then
                    ...
                    'ShowMarkets(CType(ViewState("OthpricelistRefCode2"), String))
...
#Region "Private Sub DisableAllControls()"
    Private Sub DisableAllControls()
        ...
        ElseIf ViewState("OthpricelistState2") = "Delete" Or ViewState("OthpricelistState2") = "View" Then
            ...
            TxtSellCurr.Enabled = False
            TxtSellConvRate.Enabled = False
...
#Region "Private Sub createdatarows()"
    Private Sub createdatarows()
        ...
        Try
            ...
            strSqlQry = "SELECT  count(othtypmast.othtypcode) othtypmast  WHERE (othtypmast.othgrpcode = '" & ddlGroupCode.Items(ddlGroupCode.SelectedIndex).Text & "')  and othtypmast.active=1"
            cnt = objUtils.ExecuteQueryReturnSingleValuenew(Session("dbconnectionName"), strSqlQry)
        ...
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objUtils.WritErrorLog("handlingFeesPricelist2.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        Finally
            clsDBConnect.dbCommandClose(mySqlCmd)
            clsDBConnect.dbReaderClose(mySqlReader)
            clsDBConnect.dbConnectionClose(mySqlConn)
...
#Region "Private Sub ShowDynamicGrid()"
    ...
    Private Sub ShowDynamicGrid(ByVal RefCode As String)
        Try
            ...
            '         "oplist_costd.othcatcode,oplist_costd.tcostprice,oplist_costd.frmdate,oplist_costd.todate,oplist_costd.pkgnights  " & _
                    ...
                    For Each GVROW In gv_SearchResult.Rows
                        ...
                        If GVROW.Cells(2).Text = mySqlReader("othtypcode") Then
                            StrQryTemp = "select othplist_selld.oplistcode ,othplist_selld.plgrpcode,othplist_selld.othgrpcode,othplist_selld.oclineno,  " & _
                                " othplist_selld.othtypcode, othplist_selld.othcatcode, othplist_selld.tcostprice,othplist_selld.tprice, othplist_selld.pkgnights  " & _
...
#Region "Protected Sub BtnClearFormula_Click(ByVal sender As Object, ByVal e As System.EventArgs)"
    Protected Sub BtnClearFormula_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BtnClearFormula.Click
        ...
        Try
            ...
            Response.Redirect("HandlingFeesPricelist1.aspx?&State=" & ViewState("OthpricelistState2") & "&RefCode=" & ViewState("OthpricelistRefCode2") &
"&othsellcode=" & sellingType & "&othsellname=" & sellingtypeName & "&othsellcurrCode=" & sellcurr & "&othsellconvrate=" & sellconvrate &
                    "&PlcCode=" & PlcCode & "&GroupCode=" & GroupCode & "&GroupName=" & GroupName & 
                              "&SubSeasCode=" & SubSeasCode & "&SubSeasName=" & SubSeasName &
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            If Page.IsValid = True Then
                Dim currcode As String = objUtils.ExecuteQueryReturnStringValuenew(Session("dbconnectionName"), "select option_selected from reservation_parameters where param_id=457")
                If ViewState("OthpricelistState2") = "New" Or ViewState("OthpricelistState2") = "Edit" Or ViewState("OthpricelistState2") = "Copy" Then
                    ...
                    End If
                    If CType(ddlOtherSell.Items(ddlOtherSell.SelectedIndex).Text.Trim, String) = "[Select]" Then
                        mySqlCmd.Parameters.Add(New SqlParameter("@othsellcode", SqlDbType.VarChar, 20)).Value = DBNull.Value
                    Else
                        mySqlCmd.Parameters.Add(New SqlParameter("@othsellcode", SqlDbType.VarChar, 20)).Value = CType(ddlOtherSell.Items(ddlOtherSell.SelectedIndex).Text.Trim, String)
                    End If
                    ...
                    'mySqlCmd.Parameters.Add(New SqlParameter("@currcode", SqlDbType.VarChar, 20)).Value = currcode

                    If CType(TxtSellCurr.Text.Trim, String) = "" Then
                        mySqlCmd.Parameters.Add(New SqlParameter("@currcode", SqlDbType.VarChar, 20)).Value = DBNull.Value
                    Else
                        mySqlCmd.Parameters.Add(New SqlParameter("@currcode", SqlDbType.VarChar, 20)).Value = CType(TxtSellCurr.Text.Trim, String)
                    End If
                ...
                'This code is not commented in Al_Thurya. Check with madam it is needed or not.
                ...
                '               strscript1 = "window.opener.__doPostBack('HFeesPriceListWindowPostBack', '');window.opener.focus();window.close();"
                ...
                '               Response.Redirect("HandlingFeesPricelistSellingRates2.aspx?State=" & CType(ViewState("OthpricelistState2"), String) &
                '                                 "&RefCode=" & CType(ViewState("OthpricelistRefCode2"), String) &
                '                                 "&othsellcode=" & ddlOtherSell.Items(ddlOtherSell.SelectedIndex).Text & "&othsellname=" & ddlOtherSellName.Items(ddlOtherSellName.SelectedIndex).Text & "&othsellcurrCode=" & TxtSellCurr.Text & "&othsellconvrate=" & TxtSellConvRate.Text &
                '"&SubSeasonCode=" & ddlSubSeasCode.Items(ddlSubSeasCode.SelectedIndex).Text & "&SubSeasonName=" & ddlSubSeasName.Items(ddlSubSeasName.SelectedIndex).Text & "&PListCode=" & CType(txtPlcCode.Value.Trim, String), False)
                                    ...
                                    Else
                                        If txt.Text <> "" Then
                                            If CType(Val(txt.Text), String) <> "0" Then
                                                ...
                                                'Else
                                                mySqlCmd = New SqlCommand("sp_chkHfeespldatenew", mySqlConn)
                                                'End If
                                                mySqlCmd.CommandType = CommandType.StoredProcedure
                                                ...
                                                Else
                                                    mySqlCmd.Parameters.Add(New SqlParameter("@oplistcode", SqlDbType.VarChar, 20)).Value = CType(txtPlcCode.Value.Trim, String)
                                                End If
                                                'If CType(ddlMarketCode.Items(ddlMarketCode.SelectedIndex).Text.Trim, String) = "[Select]" Then
                                                '    mySqlCmd.Parameters.Add(New SqlParameter("@plgrpcode", SqlDbType.VarChar, 20)).Value = DBNull.Value
                                                'Else
                                                '    mySqlCmd.Parameters.Add(New SqlParameter("@plgrpcode", SqlDbType.VarChar, 20)).Value = CType(ddlMarketCode.Items(ddlMarketCode.SelectedIndex).Text.Trim, String)
                                                'End If
