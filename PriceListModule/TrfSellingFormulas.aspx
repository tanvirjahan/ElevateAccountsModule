<%@ Page Title="" Language="VB" MasterPageFile="~/SubPageMaster.master" AutoEventWireup="false" CodeFile="TrfSellingFormulas.aspx.vb" Inherits="PriceListModule_TrfSellingFormulas" %>
...
<script language="javascript" type="text/javascript" >
    ...
    function FormValidation(state) {
        if ((document.getElementById("<%=ddlSellCode.ClientID%>").value == "[Select]") || (document.getElementById("<%=txtCurrencyCode.ClientID%>").value == "") || (document.getElementById("<%=ddlOperator.ClientID%>").value == "[Select]") || (document.getElementById("<%=txtFrmValue.ClientID%>").value == 0) || (document.getElementById("<%=txtFrmValue.ClientID%>").value == "") || (document.getElementById("<%=txtFormula.ClientID%>").value == "")) {
            ...
            if (state == 'New') { if (confirm('Are you sure you want to save Transfer Selling Formula?') == false) return false; }
            if (state == 'Edit') { if (confirm('Are you sure you want to update Transfer Selling Formula?') == false) return false; }
            if (state == 'Delete') { if (confirm('Are you sure you want to delete Transfer Selling Formula?') == false) return false; }
        }
    }
...
<asp:Label id="lblHeading" runat="server" Text="Add New Transfer Selling Formula" CssClass="field_heading" Width="889px"></asp:Label></TD></TR>
