...
#Region "Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If IsPostBack = False Then
            ...
            objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddluserCode, "username", "usercode", "select username,usercode from usermaster order by username", True)

            If CType(Session("custState"), String) = "New" Then
                SetFocus(txtCustomerCode)
                lblHeading.Text = "Add Allow User Booking"
                Page.Title = Page.Title + " " + "New Allow User Booking"
...
#Region "Public Function Validateamendments() As Boolean"
    Public Function Validateamendments() As Boolean
        ...
        If ddluserCode.Value = "[Select]" Then
        ...
        Try
            If Page.IsValid = True Then
                ...
                If Session("custState") = "New" Then
                    ...
                    Exit Sub
                ElseIf Session("custState") = "Edit" Then
                    ds = objUtils.ExecuteQuerySqlnew(Session("dbconnectionName"), "select isnull(max(sno),0)+1 sno from agentmast_allowuser_log(nolock) where agentcode='" & txtCustomerCode.Value & "' ")
                    ...
                    If Session("custState") = "New" Then
                        mySqlCmd = New SqlCommand("sp_update_agentmast_allowuser_log", mySqlConn, sqlTrans)
                        frmmode = 1
                    ElseIf Session("custState") = "Edit" Then
                    ...
                    mySqlCmd.Parameters.Add(New SqlParameter("@usercode", SqlDbType.VarChar, 10)).Value = CType(ddluserCode.Value, String)
                    mySqlCmd.Parameters.Add(New SqlParameter("@remarks", SqlDbType.Text)).Value = CType(txtremarks.Text.Trim, String)
                    mySqlCmd.ExecuteNonQuery()

                    mySqlCmd = New SqlCommand("sp_update_agentmast_allowuser", mySqlConn, sqlTrans)
                    mySqlCmd.CommandType = CommandType.StoredProcedure
                    mySqlCmd.Parameters.Add(New SqlParameter("@agentcode", SqlDbType.VarChar, 20)).Value = CType(txtCustomerCode.Value.Trim, String)
                    mySqlCmd.Parameters.Add(New SqlParameter("@usercode", SqlDbType.VarChar, 10)).Value = CType(ddluserCode.Value, String)
                    mySqlCmd.Parameters.Add(New SqlParameter("@sno", SqlDbType.VarChar, 20)).Value = CType(sno, String)
...
#Region "Private Sub fillgrid(ByVal RefCode As String)"
    Private Sub fillgrid(ByVal RefCode As String)
        Try
            ...
            End If
            strSqlQry = ""
            strSqlQry = "select top 100 sno,usercode,adate,requestid,remarks from agentmast_allowuser_log where agentcode='" & RefCode & "' order by sno DESC "
