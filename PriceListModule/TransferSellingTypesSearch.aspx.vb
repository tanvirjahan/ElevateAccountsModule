    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            Try
                ...
                Else
                    If Request.QueryString("Type") = "HF" Then
                        ...
                        objUser.CheckUserRight(Session("dbconnectionName"), CType(Session("GlobalUserName"), String), CType(Session("Userpwd"), String), _
                                                    CType(strappname, String), "PriceListModule\TransferSellingTypesSearch.aspx?Type=HF", btnAddNew, btnExportToExcel, _
                                                              btnPrint, gv_SearchResult, GridCol.Edit, GridCol.Delete, GridCol.View)

                    Else
                        Lblselltypes.Text = "Transfer Selling Types List"
                        objUser.CheckUserRight(Session("dbconnectionName"), CType(Session("GlobalUserName"), String), CType(Session("Userpwd"), String), _
                                                    CType(strappname, String), "PriceListModule\TransferSellingTypesSearch.aspx", btnAddNew, btnExportToExcel, _
                                                    btnPrint, gv_SearchResult, GridCol.Edit, GridCol.Delete, GridCol.View)
                ...
                Session.Add("strsortExpression", "trfsellcode")
                Session.Add("strsortdirection", SortDirection.Ascending)

                'FillGrid("othsellcode")
                fillorderby()
                FillGrid("trfsellmast.trfsellname")
                Dim typ As Type
            ...
            Catch ex As Exception
                ...
                objUtils.WritErrorLog("TransferSellingTypesSearch.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
            End Try
        End If
    ...
    Private Function BuildCondition() As String
        ...
        If txtSellingCode.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                ...
                strWhereCond = " upper(trfsellmast.trfsellcode) LIKE '" & Trim(txtSellingCode.Text.Trim.ToUpper) & "%'"
            Else
                strWhereCond = strWhereCond & " AND upper(trfsellmast.trfsellcode) LIKE '" & Trim(txtSellingCode.Text.Trim.ToUpper) & "%'"
            End If
        End If
        ...
        If txtSellingName.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                ...
                strWhereCond = " upper(trfsellmast.trfsellcode) LIKE '" & Trim(txtSellingName.Text.Trim.ToUpper) & "%'"
            Else
                strWhereCond = strWhereCond & " AND upper(trfsellmast.trfsellname) LIKE '" & Trim(txtSellingName.Text.Trim.ToUpper) & "%'"
            End If
        End If
...
#Region "Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = 'ASC')"
    ...
    Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = "ASC")
        ...
        Try
            ...
            strSqlQry = "SELECT trfsellmast.trfsellcode,trfsellmast.trfsellname, trfsellmast.currcode, currmast.currname,(select case when trfsellmast.active=1 then 'Active' else 'Inactive' end) as Active, " & _
                      "trfsellmast.adddate, trfsellmast.adduser, trfsellmast.moddate, trfsellmast.moduser " & _
                      "FROM currmast INNER JOIN " & _
                      "trfsellmast ON currmast.currcode = trfsellmast.currcode "
...
#Region "Protected Sub btnAddNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddNew.Click"
    ...
    Protected Sub btnAddNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddNew.Click
        ...
        'strpop = "window.open('TransferSellingTypes.aspx?Type=trfsell&State=New','TransferSellingTypesSearch','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
        strpop = "window.open('TransferSellingTypes.aspx?Type=trfsell&State=New','TransferSellingTypesSearch');"
    ...
    Protected Sub gv_SearchResult_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gv_SearchResult.RowCommand
        ...
        Try
            ...
            If e.CommandName = "Editrow" Then
                ...
                'Response.Redirect("Other Services Selling Types.aspx", False)
                Dim strpop As String = ""
                'strpop = "window.open('TransferSellingTypes.aspx?Type=trfsell&State=Edit&RefCode=" + CType(lblId.Text.Trim, String) + "','OtherServicesSellingTypes','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                strpop = "window.open('TransferSellingTypes.aspx?Type=trfsell&State=Edit&RefCode=" + CType(lblId.Text.Trim, String) + "','OtherServicesSellingTypes');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            ElseIf e.CommandName = "View" Then
                ...
                'Response.Redirect("Other Services Selling Types.aspx", False)
                Dim strpop As String = ""
                'strpop = "window.open('TransferSellingTypes.aspx?Type=trfsell&State=View&RefCode=" + CType(lblId.Text.Trim, String) + "','OtherServicesSellingTypes','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                strpop = "window.open('TransferSellingTypes.aspx?Type=trfsell&State=View&RefCode=" + CType(lblId.Text.Trim, String) + "','OtherServicesSellingTypes');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            ElseIf e.CommandName = "Deleterow" Then
                ...
                'Response.Redirect("Other Services Selling Types.aspx", False)
                Dim strpop As String = ""
                ' strpop = "window.open('TransferSellingTypes.aspx?Type=trfsell&State=Delete&RefCode=" + CType(lblId.Text.Trim, String) + "','OtherServicesSellingTypes','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                strpop = "window.open('TransferSellingTypes.aspx?Type=trfsell&State=Delete&RefCode=" + CType(lblId.Text.Trim, String) + "','OtherServicesSellingTypes');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            End If
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        Select Case ddlOrderBy.SelectedIndex
            Case 0
                FillGrid("trfsellmast.trfsellname")
            Case 1
                FillGrid("trfsellmast.trfsellcode")
            Case 2
                FillGrid("currmast.currcode")
    ...
    Protected Sub btnExportToExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExportToExcel.Click
        ...
        Try
            If gv_SearchResult.Rows.Count <> 0 Then
                ...
                strSqlQry = "SELECT trfsellmast.trfsellcode AS [Transfer Selling Code], trfsellmast.trfsellname AS [Transfer Selling Name], " & _
"trfsellmast.currcode AS [Currency Code], [Active]=case when trfsellmast.active =1 then 'Active'   when trfsellmast.active=0 then 'InActive' end , " & _
"(Convert(Varchar, Datepart(DD,trfsellmast.adddate))+'/'+ Convert(Varchar, Datepart(MM,trfsellmast.adddate))+ '/'+ Convert(Varchar, Datepart(YY,trfsellmast.adddate)) + ' ' + Convert(Varchar, Datepart(hh,trfsellmast.adddate))+ ':' + Convert(Varchar, Datepart(m,trfsellmast.adddate))+ ':'+ Convert(Varchar, Datepart(ss,trfsellmast.adddate))) as [Date Created],trfsellmast.adduser as [User Created],(Convert(Varchar, Datepart(DD,trfsellmast.moddate))+ '/' + Convert(Varchar, Datepart(MM,trfsellmast.moddate))+ '/'+ Convert(Varchar, Datepart(YY,trfsellmast.moddate)) + ' ' + Convert(Varchar, Datepart(hh,trfsellmast.moddate))+ ':'+ Convert(Varchar, Datepart(hh,trfsellmast.moddate))+ ':' + Convert(Varchar, Datepart(ss,trfsellmast.moddate))) as [Date Modified],trfsellmast.moduser as [User Modified]  " & _
 "FROM currmast INNER JOIN trfsellmast ON currmast.currcode = trfsellmast.currcode"



                If Trim(BuildCondition) <> "" Then
                    strSqlQry = strSqlQry & " WHERE " & BuildCondition() & " ORDER BY trfsellcode "
                Else
                    strSqlQry = strSqlQry & " ORDER BY trfsellcode"
                End If
                SqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))

                myDataAdapter = New SqlDataAdapter(strSqlQry, SqlConn)
                myDataAdapter.Fill(DS, "trfsellmast")
    ...
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        Try
            ...
            If Request.QueryString("Type") = "HF" Then
                ...
                strpop = "window.open('rptReportNew.aspx?Pageame=Handling Fees Selling Type&BackPageName=TransferSellingTypesSearch.aspx?Type=HF&SellCode=" & txtSellingCode.Text.Trim & "&SellName=" & txtSellingName.Text.Trim & "&CurrCode=" & Trim(ddlCurCode.Items(ddlCurCode.SelectedIndex).Text) & "&CurrName=" & Trim(ddlCurName.Items(ddlCurName.SelectedIndex).Text) & "','OthSellType');"
            Else
                'strpop = "window.open('rptReportNew.aspx?Pageame=TransferSellingTypesSearch&BackPageName=TransferSellingTypesSearch.aspx&trfsellcode=" & txtSellingCode.Text.Trim & "&trfsellname=" & txtSellingName.Text.Trim & "&CurrCode=" & Trim(ddlCurCode.Items(ddlCurCode.SelectedIndex).Text) & "&CurrName=" & Trim(ddlCurName.Items(ddlCurName.SelectedIndex).Text) & "','OthSellType','width=1000,height=620 left=20,top=20 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                strpop = "window.open('rptReportNew.aspx?Pageame=TransferSellingTypesSearch&BackPageName=TransferSellingTypesSearch.aspx&trfsellcode=" & txtSellingCode.Text.Trim & "&trfsellname=" & txtSellingName.Text.Trim & "&CurrCode=" & Trim(ddlCurCode.Items(ddlCurCode.SelectedIndex).Text) & "&CurrName=" & Trim(ddlCurName.Items(ddlCurName.SelectedIndex).Text) & "');"
