...
Partial Class PriceListModule_TrSellingRatesPaxSlab1
...
#Region " Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init"
    Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        ViewState.Add("TrfPLSRPaxSlabState1", Request.QueryString("State"))
        ViewState.Add("TrfPLSRPaxSlabCode1", Request.QueryString("RefCode"))
        ...
        If IsPostBack = False Then
            ...
            ShowSellingRadioBtns(ViewState("TrfPLSRPaxSlabCode1"))
            createdatatable()
            dt = Session("TrfPSGridData")
            Dim fld2 As String = ""
...
#Region "Private Sub createdatatable()"
    ...
    Private Sub createdatatable()
        Try
            ...
            strSqlQry = " SELECT  count(othcat_slabs.slabno ) FROM othcat_slabs inner join othcatmast on othcat_slabs .othcatcode =othcatmast.othcatcode " & _
                " INNER JOIN partyothcat ON dbo.partyothcat.othcatcode = dbo.othcatmast.othcatcode WHERE (dbo.othcatmast.othgrpcode = '" & ddlGroupCode.Items(ddlGroupCode.SelectedIndex).Text & "') and " & _
                " (partyothcat.partycode='" & ddlSupplierCode.Items(ddlSupplierCode.SelectedIndex).Text & "') and othcatmast.active=1"
            ...
            'strSqlQry = "SELECT  distinct dbo.othcatmast.othcatcode, dbo.othcatmast.othgrpcode,othcatmast.grporder FROM dbo.partyothcat INNER JOIN
            'dbo.othcatmast ON dbo.partyothcat.othcatcode = dbo.othcatmast.othcatcode WHERE (dbo.othcatmast.othgrpcode = '" & ddlGroupCode.Items(ddlGroupCode.SelectedIndex).Text & "') 
            'and (partyothcat.partycode='" & ddlSupplierCode.Items(ddlSupplierCode.SelectedIndex).Text & "') and othcatmast.active=1 Order by othcatmast.grporder"

            strSqlQry = " SELECT  distinct othcat_slabs.slabno, othcat_slabs.othgrpcode,othcat_slabs.othcatcode  FROM othcat_slabs " & _
                " inner join othcatmast on othcat_slabs .othcatcode =othcatmast.othcatcode INNER JOIN partyothcat ON dbo.partyothcat.othcatcode = dbo.othcatmast.othcatcode " & _
                "  WHERE (othcat_slabs.othgrpcode = '" & ddlGroupCode.Items(ddlGroupCode.SelectedIndex).Text & "')  and (partyothcat.partycode='" & ddlSupplierCode.Items(ddlSupplierCode.SelectedIndex).Text & "')  and othcatmast.active=1 Order by othcat_slabs.slabno "


            mySqlCmd = New SqlCommand(strSqlQry, mySqlConn)
            mySqlReader = mySqlCmd.ExecuteReader()
            While mySqlReader.Read = True
                arr(Column) = mySqlReader("slabno")
                Column = Column + 1
            End While
        ...
        Catch ex As Exception
            ...
            objUtils.WritErrorLog("TrfSellingRatePaxSlab.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        Finally
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Page.IsPostBack = False Then
            Try
                ...
                ShowDates(ViewState("TrfPLSRPaxSlabCode1"))
                ...
                Try
                    ...
                    If ViewState("TrfPLSRPaxSlabState1") = "New" Then
                        'lblHeading.Text = "Add Price List - Selling Rates"
                        lblHeading.Text = "Transfer  Price List - Selling Rates Pax Slab"
                        Page.Title = Page.Title + " " + "Transfer Price List - Selling Rates Pax Slab"
                        ...
                        '    gv_SearchResult.Enabled = False
                        'End If
                    ElseIf ViewState("TrfPLSRPaxSlabState1") = "Edit" Then
                        lblHeading.Text = "Transfer  Price List - Selling Rates Pax Slab"
                        Page.Title = Page.Title + " " + "Transfer  Price List - Selling Rates Pax Slab"
                        ...
                        ''    gv_SearchResult.Enabled = False
                        ''End If
                    ElseIf ViewState("TrfPLSRPaxSlabState1") = "View" Then
                        lblHeading.Text = "Transfer  Price List - Selling Rates Pax Slab"
                        Page.Title = Page.Title + " " + "Transfer  Price List - Selling Rates Pax Slab"
                        ShowDynamicGrid()
                        btnSave.Visible = False
                        gv_SearchResult.Enabled = False
                    ElseIf ViewState("TrfPLSRPaxSlabState1") = "Copy" Then
                        lblHeading.Text = "Transfer  Price List - Selling Rates Pax Slab"
                        Page.Title = Page.Title + " " + "Transfer  Price List - Selling Rates Pax Slab"
                        ''btnGenerate.Visible = False
                        ''If ChkBManual.Checked = True Then
...
#End Region
    ...
    Private Sub ShowDynamicGrid()
        ...
        Try
            ...
            'Else
            StrQry = "select distinct oclineno from trfplist_othcat_slabs where tplistcode='" & txtPLCode.Text & "' and selltype='" & lblSellingCode.Text.Trim & "'"
            'End If
            mySqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
                ...
                While mySqlReader.Read
                    For Each gvrow In gv_SearchResult.Rows
                        ...
                        If gvrow.Cells(0).Text = Linno Then
                            convrate = objUtils.GetDBFieldFromMultipleCriterianew(Session("dbconnectionName"), "trfplisth_convrates", "convrate", "tplistcode='" & txtPLCode.Text & "' and sellcode='" & lblSellingCode.Text.Trim & "'")
                            'If ddlPriceList.SelectedValue = "Weekend Rates 1 Night" Or ddlPriceList.SelectedValue = "Weekend Rates > 1 Night" Then
                            ...
                            'Else
                            StrQryTemp = "select * from trfplist_othcat_slabs where tplistcode='" & txtPLCode.Text & "' and selltype='" & lblSellingCode.Text.Trim & "'and oclineno='" & Linno & "'"
                            'End If
                            ...
                            If myReader.HasRows Then
                                While myReader.Read
                                    If IsDBNull(myReader("slabno")) = False Then
                                        othcatcode = myReader("slabno")
                                    End If
                                    If IsDBNull(myReader("slabsellprice")) = False Then
                                        value = myReader("slabsellprice")
                                    Else
                                        value = ""
                                    End If
                                    If IsDBNull(myReader("slabcostprice")) = False Then
                                        cvalue = myReader("slabcostprice")
                                    Else
                                        cvalue = ""
                                    End If
                                    For j = 0 To cnt - 4
...
go1:                            End While
            ...
            'Merge catcode in the header
            For header = 0 To cnt - 4
                txt = gv_SearchResult.HeaderRow.FindControl("txtHead" & header)
                If txt.Text <> Nothing Then

                    StrQry = "select top 1 othcatcode from trfplist_othcat_slabs where tplistcode='" & txtPLCode.Text & "' and selltype='" & lblSellingCode.Text.Trim & "' and slabno='" + CType(txt.Text, String) + "'"
                    mySqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
                    mySqlCmd = New SqlCommand(StrQry, mySqlConn)
                    mySqlReader = mySqlCmd.ExecuteReader()
                    If mySqlReader.HasRows Then
                        While mySqlReader.Read
                            ...
                            If IsDBNull(mySqlReader("othcatcode")) = False Then
                                If txt.Text <> "From Date" Or txt.Text <> "To Date" Or txt.Text <> "Pkg" Then
                                    txt.Text = "Slab " + txt.Text + Chr(13) + mySqlReader("othcatcode")
        ...
        Catch ex As Exception
            'ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objUtils.WritErrorLog("TrSellingRatesPaxSlab1.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
            clsDBConnect.dbConnectionClose(mySqlConn)
        End Try
    End Sub
...
#Region "Private Sub ShowRecord(ByVal RefCode As String)"
    Private Sub ShowRecord(ByVal RefCode As String)
            ...
            If mySqlReader.HasRows Then
                If mySqlReader.Read() = True Then
                    If IsDBNull(mySqlReader("tplistcode")) = False Then
                        ...
                        txtPLCode.Text = mySqlReader("tplistcode")
                    End If
        ...
        Catch ex As Exception
            objUtils.WritErrorLog("TrfPricelistSellingRates1.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        Finally
...
#End Region
    ...
    Protected Sub btnClear_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnClear.Click
        ...
        Response.Redirect("TrfSellingRatePaxSlab.aspx?State=" & CType(ViewState("TrfPLSRPaxSlabState1"), String) & "&RefCode=" & CType(ViewState("TrfPLSRPaxSlabCode1"), String) &
                          ...
                          "&PListCode=" & txtPLCode.Text & "&sellcode=" & rdSellingList.SelectedValue & "&week1=" & week1 &
                          "&week2=" & week2 & "&manual=" & manual, False)
    End Sub
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        '    Response.Redirect("TrfPricelistSellingRates.aspx?State=" & CType(ViewState("TrfPLSRPaxSlabState1"), String) & "&RefCode=" & CType(ViewState("TrfPLSRPaxSlabCode1"), String) &
        '                      "&supplier=" & ddlSupplierCode.Items(ddlSupplierCode.SelectedIndex).Text & "&suppliername=" & ddlSupplierName.Items(ddlSupplierName.SelectedIndex).Text &
    ...
    Private Sub ShowDates(ByVal RefCode As String)
        ...
        Catch ex As Exception
            objUtils.WritErrorLog("TrfPricelistSellingRate.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
        Finally
            clsDBConnect.dbCommandClose(mySqlCmd)               'sql command disposed
    ...
    Public Sub fillDategrd(ByVal grd As GridView, Optional ByVal blnload As Boolean = False, Optional ByVal count As Integer = 1)
        Dim lngcnt As Long
        If ViewState("TrfPLSRPaxSlabCode1") <> Nothing Then
            mySqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
            strSqlQry = "select count(*) from trfplisth_dates where tplistcode='" + ViewState("TrfPLSRPaxSlabCode1") + "'"
            mySqlCmd = New SqlCommand(strSqlQry, mySqlConn)
    ...
    Private Sub FillGridAsPrices()
        ...
        Try
            ...
            '    StrQry = "select distinct clineno from cplistdwknew where plistcode='" & txtBlockCode.Value & "' and selltype='" & lblSellingCode.Text.Trim & "'"
            'Else
            StrQry = "select distinct oclineno from trfplist_selld where tplistcode='" & txtPLCode.Text & "' and selltype='" & lblSellingCode.Text.Trim & "'"
            'End If
            mySqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))
            mySqlCmd = New SqlCommand(StrQry, mySqlConn)
            mySqlReader = mySqlCmd.ExecuteReader()
...
#Region "Private Sub FillGridMarket(ByVal strorderby As String, Optional ByVal strsortorder As String = 'ASC')"
    Private Sub FillGridMarket(ByVal strorderby As String, Optional ByVal strsortorder As String = "ASC")
        ...
        Try
            ...
            strSqlQry = strSqlQry & " and plgrpcode in (select plgrpcode from trfplist_selld  where tplistcode ='" + txtPLCode.Text.Trim + "') ORDER BY " & strorderby & " " & strsortorder
