'------------================--------------=======================------------------================
'   Page Name       :   CurrenciesSearch.aspx
...
Partial Class CurrenciesSearch
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Page.IsPostBack = False Then
            ...
            Try
                ...
                Else
                    objUser.CheckUserRight(Session("dbconnectionName"), CType(Session("GlobalUserName"), String), CType(Session("Userpwd"), String), _
                                                       CType(strappname, String), "PriceListModule\CurrenciesSearch.aspx?appid=" + strappid, btnAddNew, btnExportToExcel, _
            ...
            Catch ex As Exception
                ...
                objUtils.WritErrorLog("CurrenciesSearch.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
            End Try
        End If
...
#Region "  Private Function BuildCondition() As String"
    Private Function BuildCondition() As String
        strWhereCond = ""
        If txtCurrenciesCode.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then

                strWhereCond = " upper(currmast.currcode) LIKE '" & Trim(txtCurrenciesCode.Text.Trim.ToUpper) & "%'"
            Else
                strWhereCond = strWhereCond & " AND upper(currmast.currcode) LIKE '" & Trim(txtCurrenciesCode.Text.Trim.ToUpper) & "%'"
            End If
        End If

        If txtCurrenciesName.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then

                strWhereCond = " upper(currmast.currname) LIKE '" & Trim(txtCurrenciesName.Text.Trim.ToUpper) & "%'"
            Else
                strWhereCond = strWhereCond & " AND upper(currmast.currname) LIKE '" & Trim(txtCurrenciesName.Text.Trim.ToUpper) & "%'"
            End If
        End If
...
#Region "Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = 'ASC')"
    Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = "ASC")
        ...
        Dim pagevalue1 = getRowpage()
        If gv_SearchResult.PageIndex < 0 Then
            gv_SearchResult.PageIndex = 0
        End If
        strSqlQry = ""
        Try
            strSqlQry = "SELECT *,[IsActive]=case when active=1 then 'Active' when active=0 then 'InActive'end  FROM currmast"
            ...
            If myDS.Tables(0).Rows.Count > 0 Then
                gv_SearchResult.PageSize = pagevalue1
...
#Region "Protected Sub btnAddNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddNew.Click"
    Protected Sub btnAddNew_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnAddNew.Click
        ...
        strpop = "window.open('Currencies.aspx?State=New','Currencies');"
    ...
    Protected Sub gv_SearchResult_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gv_SearchResult.RowCommand
        Try
            ...
            If e.CommandName = "EditRow" Then
                ...
                'Response.Redirect("Currencies.aspx", False)
                Dim strpop As String = ""
                'strpop = "window.open('Currencies.aspx?State=Edit&RefCode=" + CType(lblId.Text.Trim, String) + "','Currencies','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                strpop = "window.open('Currencies.aspx?State=Edit&RefCode=" + CType(lblId.Text.Trim, String) + "','Currencies');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            ElseIf e.CommandName = "View" Then
                ...
                'Response.Redirect("Currencies.aspx", False)
                Dim strpop As String = ""
                'strpop = "window.open('Currencies.aspx?State=View&RefCode=" + CType(lblId.Text.Trim, String) + "','Currencies','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                strpop = "window.open('Currencies.aspx?State=View&RefCode=" + CType(lblId.Text.Trim, String) + "','Currencies');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            ElseIf e.CommandName = "DeleteRow" Then
                ...
                'Response.Redirect("Currencies.aspx", False)
                Dim strpop As String = ""
                'strpop = "window.open('Currencies.aspx?State=Delete&RefCode=" + CType(lblId.Text.Trim, String) + "','Currencies','width='+screen.availWidth+' height='+screen.availHeight+' left=0,top=0 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
                strpop = "window.open('Currencies.aspx?State=Delete&RefCode=" + CType(lblId.Text.Trim, String) + "','Currencies');"
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
            End If
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
...
#Region "Protected Sub btnClear_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnClear.Click"
    Protected Sub btnClear_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnClear.Click
        txtCurrenciesCode.Text = ""
        txtCurrenciesName.Text = ""
        'FillGrid("currcode")
                ...
                If dtDynamics.Rows.Count > 0 Then
                    ...
                    For j = 0 To dtDynamics.Rows.Count - 1
                        ...
                        If lsCurrencyName.Trim <> "" Then
                            lblCurrencyName.Text = Regex.Replace(lblCurrencyName.Text.Trim, lsCurrencyName.Trim(), _
    ...
    Protected Sub btnExportToExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnExportToExcel.Click
        ...
        Try
            If gv_SearchResult.Rows.Count <> 0 Then
                ...
                'Response.End()
                'Response.Clear()
                strSqlQry = "SELECT currcode AS [Currency Code],currname AS [Currency Name],currcoin as [Currency Coin],currmast.convrate as [Exchange Rate],[Active]=case when active=1 then 'Active' when active=0 then 'InActive' end,(Convert(Varchar, Datepart(DD,adddate))+ '/'+ Convert(Varchar, Datepart(MM,adddate))+ '/'+ Convert(Varchar, Datepart(YY,adddate)) + ' ' + Convert(Varchar, Datepart(hh,adddate))+ ':' + Convert(Varchar, Datepart(m,adddate))+ ':'+ Convert(Varchar, Datepart(ss,adddate))) as [Date Created],moduser as [User Modified] ,(Convert(Varchar, Datepart(DD,moddate))+ '/'+ Convert(Varchar, Datepart(MM,moddate))+ '/'+ Convert(Varchar, Datepart(YY,moddate)) + ' ' + Convert(Varchar, Datepart(hh,moddate))+ ':' + Convert(Varchar, Datepart(m,moddate))+ ':'+ Convert(Varchar, Datepart(ss,moddate))) as [Date Modified] FROM currmast"

                If Trim(BuildCondition) <> "" Then
                    strSqlQry = strSqlQry & " WHERE " & BuildCondition() & " ORDER BY currcode "
                Else
                    strSqlQry = strSqlQry & " ORDER BY currcode"
                End If
...
#Region "Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click"
    Protected Sub btnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        Try
            ...
            'Session.Add("CurrencyCode", txtCurrenciesCode.Text.Trim)
            'Session.Add("CurrencyName", txtCurrenciesName.Text.Trim)
            'Response.Redirect("rptCurrencies.aspx", False)
            Dim strpop As String = ""
            'strpop = "window.open('rptCurrencies.aspx?CurrencyCode=" + txtCurrenciesCode.Text.Trim + "&CurrencyName=" + txtCurrenciesName.Text.Trim + "','RepCurrencies','width=1000,height=620 left=20,top=20 status=yes,toolbar=no,menubar=no,resizable=yes,scrollbars=yes');"
            strpop = "window.open('rptCurrencies.aspx?CurrencyCode=" + txtCurrenciesCode.Text.Trim + "&CurrencyName=" + txtCurrenciesName.Text.Trim + "','RepCurrencies');"
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "popup", strpop, True)
        Catch ex As Exception
            ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
            objUtils.WritErrorLog("CurrenciesSearch.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
...
#End Region
    Private Sub fillorderby()
        ...
        ddlOrderBy.Items.Add("Currency Name")
            ...
            For i = 0 To lsMainArr.GetUpperBound(0)
                ...
                If strValue <> "" Then
                    If Trim(strWhereCond1) = "" Then
                        strWhereCond1 = "(upper(currmast.currname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%')"
                        'Else
  ...
  If txtFromDate.Text.Trim <> "" And txtToDate.Text <> "" Then
            ...
            If ddlOrder.SelectedValue = "C" Then
                If Trim(strWhereCond) = "" Then
                    ...
                    strWhereCond = " (CONVERT(datetime, convert(varchar(10),currmast.adddate,103),103)  between CONVERT(datetime, '" + txtFromDate.Text + "',103) and CONVERT(datetime,  '" + txtToDate.Text + "',103)) "
                Else
                    strWhereCond = strWhereCond & "  and (CONVERT(datetime, convert(varchar(10),currmast.adddate,103),103)  between CONVERT(datetime, '" + txtFromDate.Text + "',103) and CONVERT(datetime,  '" + txtToDate.Text + "',103)) "
                End If
