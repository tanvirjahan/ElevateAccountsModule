...
Partial Class UploadBannerAdds
        ...
        If Page.IsPostBack = False Then
            Try
                ...
                If ViewState("UploadBannerState") = "New" Then
                    ...
                    lblHeading.Text = "Add New Banner Adds"
                    btnSave.Text = "Save"
                    btnSave.Attributes.Add("onclick", "javascript:if(confirm('Are you sure you want to save upload banner adds?')==false)return false;")
                ElseIf ViewState("UploadBannerState") = "Edit" Then
                    SetFocus(txtAlternateText)
                    lblHeading.Text = "Edit Banner Adds"
                    btnSave.Text = "Update"
                    DisableControl()
                    ShowRecord(CType(ViewState("UploadBannerRefCode"), String))
                    btnSave.Attributes.Add("onclick", "javascript:if(confirm('Are you sure you want to update upload banner adds?')==false)return false;")
                ElseIf ViewState("UploadBannerState") = "View" Then
                    SetFocus(btnCancel)
                    lblHeading.Text = "View Banner Adds"
                ...
                ElseIf ViewState("UploadBannerState") = "Delete" Then
                    SetFocus(btnSave)
                    lblHeading.Text = "Delete Banner Adds"
                    btnSave.Text = "Delete"
                    DisableControl()

                    ShowRecord(CType(ViewState("UploadBannerRefCode"), String))
                    btnSave.Attributes.Add("onclick", "javascript:if(confirm('Are you sure you want to delete upload banner adds?')==false)return false;")
                End If
...
#Region " Private Sub ShowRecord(ByVal RefCode As String)"
    ...
    Private Sub ShowRecord(ByVal RefCode As String)
        ...
        Try
            ...
            mySqlCmd = New SqlCommand("Select * from BannerAds Where bannerid='" & RefCode & "'", mySqlConn)
            ...
            If mySqlReader.HasRows Then
                If mySqlReader.Read() = True Then
                    ...
                    If IsDBNull(mySqlReader("ImageUrl")) = False Then
                        Me.txtImage.Text = CType(mySqlReader("ImageUrl"), String)
        ...
        Try
            ...
            If Page.IsValid = True Then
                If ViewState("UploadBannerState") = "New" Or ViewState("UploadBannerState") = "Edit" Then
                    ...
                    If ViewState("UploadBannerState") = "New" Then
                        mySqlCmd = New SqlCommand("sp_add_BannerAds", mySqlConn, sqlTrans)
                        ...
                        mySqlCmd.Parameters.Add(New SqlParameter("@ImageUrl", SqlDbType.VarChar, 100)).Value = CType(txtImage.Text.Trim, String)
                        mySqlCmd.Parameters.Add(New SqlParameter("@NavigateUrl", SqlDbType.VarChar, 200)).Value = CType(txtNavigateUrl.Text.Trim, String)
                    ElseIf ViewState("UploadBannerState") = "Edit" Then
                        mySqlCmd = New SqlCommand("sp_mod_BannerAds", mySqlConn, sqlTrans)
                        mySqlCmd.CommandType = CommandType.StoredProcedure
                ...
                ElseIf ViewState("UploadBannerState") = "Delete" Then
                    mySqlConn = clsDBConnect.dbConnectionnew(Session("dbconnectionName"))           'connection open
                    sqlTrans = mySqlConn.BeginTransaction           'SQL  Trans start
                    mySqlCmd = New SqlCommand("sp_delete_BannerAds", mySqlConn, sqlTrans)
                    mySqlCmd.CommandType = CommandType.StoredProcedure
                    mySqlCmd.Parameters.Add(New SqlParameter("@bannerid", SqlDbType.Int, 4)).Value = CType(txtBannerId.Text.Trim, Integer)
                    mySqlCmd.ExecuteNonQuery()
                ...
                strscript = "window.opener.__doPostBack('UploadBannerWindowPostBack', '');window.opener.focus();window.close();"
            ...
            If fuImage.HasFile Then
                ...
                If ext.ToUpper = "GIF" Or ext.ToUpper = "JPEG" Or ext.ToUpper = "JPG" Then
                    ...
                    If CheckImage() = False Then
                        ...
                        Exit Sub
                    Else
                        SendImageToWebService(Server.MapPath("~/AgentModule/UploadedImages/" + strFileName), strFileName)
                    End If
                    ImgBanner.ImageUrl = txtImage.Text
                Else
