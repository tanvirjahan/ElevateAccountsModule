...
Partial Class WebAdminModule_PwdMailSendLog
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Page.IsPostBack = False Then
            Try
                ...
                End If
                Session.Add("strsortExpression", "SLNO")
                Session.Add("strsortdirection", SortDirection.Ascending)
                FillGrid("SLNO")
            Catch ex As Exception
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
                objUtils.WritErrorLog("PwdMailSendLog.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
            End Try
        End If
        btnClear.Attributes.Add("onclick", "javascript:if(confirm('Are you sure you want to clear search criteria?')==false)return false;")

        ClientScript.GetPostBackEventReference(Me, String.Empty)
        If (Me.IsPostBack And Me.Request("__EVENTTARGET") = "UploadImageWebWindowPostBack") Then
...
#Region "  Private Function BuildCondition() As String"
    Private Function BuildCondition() As String
        strWhereCond = ""
        If txtagentcodeId.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then

                strWhereCond = " p.agentcode = '" & Trim(txtagentcodeId.Text.Trim) & "'"
            Else
                strWhereCond = strWhereCond & " AND p.agentcode  = '" & Trim(txtagentcodeId.Text.Trim) & "'"
            End If
        End If
        ...
        If txtagentName.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                ...
                strWhereCond = "a.agentname LIKE  '" & Trim(txtagentName.Text.Trim.ToUpper) & "%'"
            Else
                strWhereCond = strWhereCond & " AND a.agentname LIKE  '" & Trim(txtagentName.Text.Trim.ToUpper) & "%'"
            End If
        End If
        ...
        If txtUserName.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                ...
                strWhereCond = " case when isnull(u.UserName,'')='' then a.agentname else u.UserName end  LIKE  '" & Trim(txtUserName.Text.Trim.ToUpper) & "%'"
            Else
                strWhereCond = strWhereCond & " AND  case when isnull(u.UserName,'')='' then a.agentname else u.UserName end  LIKE  '" & Trim(txtUserName.Text.Trim.ToUpper) & "%'"
            End If
        End If
...
#Region "Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = 'ASC')"
    Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = "ASC")
        ...
        strSqlQry = ""
        Try
            strSqlQry = "select SLNO , p.agentcode ,a.agentname ,p.usercode,"
            strSqlQry += " case when isnull(u.UserName,'')='' then a.agentname else u.UserName end username"
            strSqlQry += " ,mailDateTime ,mailSendPageName ,agent_email  from pwdSendMail_Log p "
            strSqlQry += " inner join agentmast a on p.agentcode =a.agentcode left outer join UserMaster u on"
...
#End Region
    ...
    Protected Sub btnHelp_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnHelp.Click
        ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "window.open('../Help.aspx?hi=PwdSendMail_log','_blank','status=1,scrollbars=1,top=135,left=760,width=250,height=500');", True)
    End Sub
