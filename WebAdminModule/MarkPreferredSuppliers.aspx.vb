...
Partial Class MarkPreferredSuppliers
            ...
            If Page.IsPostBack = False Then
                ...
                Else
                    objUser.CheckUserRight(Session("dbconnectionName"), CType(Session("GlobalUserName"), String), CType(Session("Userpwd"), String), _
                                                     CType(strappname, String), "WebAdminModule\MarkPreferredSuppliers.aspx", btnAddNew, BtnExportToExcel, _
                ...
                '--------end
                Session.Add("strsortExpression", "partycode")
                ...
                '    If hdnsuppliertype.Value <> "[Select]" And hdnsuppliertype.Value <> "" Then
                '        ddlSupplierTypeName.Value = hdnsuppliertype.Value
                '        ddlSupplierType.Value = ddlSupplierTypeName.Items(ddlSupplierTypeName.SelectedIndex).Text
                '    End If
                '    If ddlSupplierType.Value <> "[Select]" Then
                '        objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlCategory, "catcode", "catname", "select distinct partymast.catcode, catname from catmast, partymast where partymast.catcode = catmast.catcode and partymast.active=1 and partymast.sptypecode='" & ddlSupplierType.Items(ddlSupplierType.SelectedIndex).Text & "' order by partymast.catcode", True, ddlCategory.Value)
                '        objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlCategoryName, "catname", "catcode", "select distinct partymast.catcode, catname from catmast, partymast where partymast.catcode = catmast.catcode and partymast.active=1 and partymast.sptypecode='" & ddlSupplierType.Items(ddlSupplierType.SelectedIndex).Text & "' order by catname", True, ddlCategoryName.Value)
                '    Else
                '        objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlCategory, "catcode", "catname", "select distinct partymast.catcode, catname from catmast, partymast where partymast.catcode = catmast.catcode and partymast.active=1 order by partymast.catcode", True, ddlCategory.Value)
                '        objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlCategoryName, "catname", "catcode", "select distinct partymast.catcode, catname from catmast, partymast where partymast.catcode = catmast.catcode and partymast.active=1 order by catname", True, ddlCategoryName.Value)
                '    End If
                '    If hdncategory.Value <> "[Select]" And hdncategory.Value <> "" Then
                ...
                '    strSqlQry = "select distinct partymast.sectorcode, sectorname from sectormaster, partymast where partymast.sectorcode = sectormaster.sectorcode and partymast.active=1 "
                '    strSqlQry1 = "select distinct partymast.sectorcode, sectorname from sectormaster, partymast where partymast.sectorcode = sectormaster.sectorcode and partymast.active=1"
                ...
                '        strSqlQry = strSqlQry & " and partymast.citycode='" & ddlCity.Items(ddlCity.SelectedIndex).Text & "'"
                '        strSqlQry1 = strSqlQry1 & "and partymast.citycode='" & ddlCity.Items(ddlCity.SelectedIndex).Text & "'"
                '    End If
                '    strSqlQry = strSqlQry & " order by partymast.sectorcode"
                '    strSqlQry1 = strSqlQry1 & " order by sectorname"
                '    objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlSector, "sectorcode", "sectorname", strSqlQry, True, ddlSector.Value)
                '    objUtils.FillDropDownListHTMLNEW(Session("dbconnectionName"), ddlSectorName, "sectorname", "sectorcode", strSqlQry1, True, ddlSectorName.Value)

                '    If hdnsector.Value <> "[Select]" And hdnsector.Value <> "" Then
                '        ddlSectorName.Value = hdnsector.value
                '        ddlSector.Value = ddlSectorName.Items(ddlSectorName.SelectedIndex).Text
                '    End If
                FillGridNew()
    ...
    Private Function BuildConditionNew(ByVal strSectorValue As String, ByVal strCountryValue As String, ByVal strCityValue As String, ByVal strCategoryValue As String, ByVal strHotelstatusValue As String, ByVal strHotelChainValue As String, ByVal strHotelGroupValue As String, ByVal strTextValue As String) As String
        ...
        If strCountryValue.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                strWhereCond = "upper(c.ctryname) IN (" & Trim(strCountryValue.Trim.ToUpper) & ")"
            Else
                strWhereCond = strWhereCond & " AND upper(c.ctryname) IN (" & Trim(strCountryValue.Trim.ToUpper) & ")"
            End If
        End If
        If strSectorValue <> "" Then

            If Trim(strWhereCond) = "" Then
                strWhereCond = " upper(s.sectorname) IN( " & Trim(strSectorValue) & ")"
            Else

                strWhereCond = strWhereCond & " AND upper(s.sectorname) IN (" & Trim(strSectorValue) & ")"
            End If
        End If
        ...
        If strCategoryValue.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                strWhereCond = "upper(ca. catname) IN (" & Trim(strCategoryValue.Trim.ToUpper) & ")"
            Else
                strWhereCond = strWhereCond & " AND upper(ca. catname) IN (" & Trim(strCategoryValue.Trim.ToUpper) & ")"
            End If
        End If
        ...
        If strHotelGroupValue <> "" Then
            If Trim(strWhereCond) = "" Then
                strWhereCond = " p.partycode in (select cgd.partycode   from hotelgroup cg,hotelgroup_detail cgd where cg.hotelgroupcode = cgd.hotelgroupcode and hotelgroupname IN (" & Trim(strHotelGroupValue) & "))"
            Else

                strWhereCond = strWhereCond & " AND p.partycode in (select cgd.partycode   from hotelgroup cg,hotelgroup_detail cgd where cg.hotelgroupcode = cgd.hotelgroupcode and hotelgroupname IN (" & Trim(strHotelGroupValue) & "))"
            End If
        End If
            ...
            For i = 0 To lsMainArr.GetUpperBound(0)
                ...
                If strValue <> "" Then
                    If Trim(strWhereCond1) = "" Then
                        strWhereCond1 = "upper(p.partyname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or upper(c.ctryname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or upper(ct.cityname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or upper(ca.catname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' "
                    Else
                        strWhereCond1 = strWhereCond1 & " OR upper(p.partyname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or upper(c.ctryname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or upper(ct.cityname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' or upper(ca.catname) LIKE '%" & Trim(strValue.Trim.ToUpper) & "%' "
                    End If
                End If
            Next
            If Trim(strWhereCond) = "" Then
...
#End Region
                        ...
                        If lsCtryName.Trim <> "" Then
                            lblcountry.Text = Regex.Replace(lblcountry.Text.Trim, lsCtryName.Trim(), _
                                Function(match As Match) String.Format("<span style = 'background-color:#ffcc99'>{0}</span>", match.Value), _
                                            RegexOptions.IgnoreCase)
                        End If
                        If lsHotelName.Trim <> "" Then
                            lblhotelgroup.Text = Regex.Replace(lblhotelgroup.Text.Trim, lsHotelName.Trim(), _
                                Function(match As Match) String.Format("<span style = 'background-color:#ffcc99'>{0}</span>", match.Value), _
                                            RegexOptions.IgnoreCase)
                        End If
    ...
    Protected Sub BtnPrint_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnPrint.Click
        Try
            ...
            strpop = "window.open('rptReportNew.aspx?Pageame=PreferredSupplier&BackPageName=MarkPreferredSuppliers.aspx','PrefSup');"
    ...
    Private Sub FilterGrid()
        ...
        For i = 0 To lsMainArr.GetUpperBound(0)
            Select Case lsMainArr(i).Split(":")(0).ToString.ToUpper.Trim
                ...
                Case "HOTELGROUP"
                    lsProcessCat = lsMainArr(i).Split(":")(1)
                    sbAddToDataTable("HOTELGROUP", lsProcessCat, "HG")
                Case "HOTELSTATUS"
                    lsProcessCat = lsMainArr(i).Split(":")(1)
                    sbAddToDataTable("HOTELSTATUS", lsProcessCat, "HS")
                Case "HOTELCHAIN"
                    lsProcessCat = lsMainArr(i).Split(":")(1)
                    sbAddToDataTable("HOTELCHAIN", lsProcessCat, "HC")
    ...
    Protected Sub btnSave_Click(sender As Object, e As System.EventArgs) Handles btnSave.Click
        Try
            ...
            If Page.IsValid = True Then
                ...
                For Each gvRow In gv_SearchResult.Rows
                    chkSel = gvRow.FindControl("chkSelect")
                    myCommand = New SqlCommand("sp_update_partymast", SqlConn, sqlTrans)
                    myCommand.CommandType = CommandType.StoredProcedure
                    myCommand.Parameters.Add(New SqlParameter("@partycode", SqlDbType.VarChar, 20)).Value = CType(gvRow.Cells(1).Text, String)
                    If chkSel.Checked = True Then
