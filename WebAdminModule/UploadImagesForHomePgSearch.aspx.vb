...
Partial Class WebAdminModule_UploadImagesForHomePgSearch
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Page.IsPostBack = False Then
            Try
                ...
                End If
                Session.Add("strsortExpression", "imageid")
                Session.Add("strsortdirection", SortDirection.Ascending)
                FillGrid("imageid")
            Catch ex As Exception
                ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "alert('" & "Error Description " + ex.Message.Replace("'", " ") & "' );", True)
                objUtils.WritErrorLog("UploadImagesForHomePg.aspx", Server.MapPath("ErrorLog.txt"), ex.Message.ToString, Session("GlobalUserName"))
            End Try
        End If
    ...
    Private Function BuildCondition() As String
        ...
        If txtImageId.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                ...
                strWhereCond = "autoid = '" & Trim(txtImageId.Text.Trim.ToUpper) & "'"
            Else
                strWhereCond = strWhereCond & " AND autoid = '" & Trim(txtImageId.Text.Trim.ToUpper) & "'"
            End If
        End If
        ...
        If txtImageName.Text.Trim <> "" Then
            If Trim(strWhereCond) = "" Then
                ...
                strWhereCond = "imagename LIKE  '" & Trim(txtImageName.Text.Trim.ToUpper) & "%'"
            Else
                strWhereCond = strWhereCond & " AND imagename LIKE  '" & Trim(txtImageName.Text.Trim.ToUpper) & "%'"
            End If
        End If

        If ddlImgPostion.SelectedValue <> "[Select]" Then
            If Trim(strWhereCond) = "" Then

                strWhereCond = "imageposition ='" & Trim(ddlImgPostion.SelectedValue) & "'"
            Else
                strWhereCond = strWhereCond & " AND imageposition ='" & Trim(ddlImgPostion.SelectedValue) & "'"
            End If
        End If
...
#Region "Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = 'ASC')"
    Private Sub FillGrid(ByVal strorderby As String, Optional ByVal strsortorder As String = "ASC")
        ...
        strSqlQry = ""
        Try
            strSqlQry = "SELECT autoid imageid,imagename ,imageposition=case when imageposition='L' then 'LEFT'  when imageposition='R' then 'RIGHT' end  ,active ,rankorder FROM homepageimages"
...
#End Region
    ...
    Protected Sub btnHelp_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnHelp.Click
        ScriptManager.RegisterClientScriptBlock(Page, GetType(Page), "", "window.open('../Help.aspx?hi=UploadImagesForHomePgSearch','_blank','status=1,scrollbars=1,top=135,left=760,width=250,height=500');", True)
    End Sub
End Class
